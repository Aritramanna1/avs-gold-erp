...
11. **SUPABASE EGRESS & REQUEST EFFICIENCY (MANDATORY):** Follow `docs/MASTER/SUPABASE_EGRESS_ENGINEERING_RULES.md` — one login = one boot, dedupe auth/subscription, no secrets at boot, realtime after sync, R2 file path, per-login egress measurement, hard before/after acceptance. Do not upgrade Supabase compute before proving workload optimization.
...
- **Security & Infrastructure:** `SESSION_AND_SECURITY_MASTER.md`, `BACKUP_RESTORE_MASTER.md`, `PERFORMANCE_ARCHITECTURE.md`, `DATABASE_AND_SUPABASE_MASTER.md`, **`SUPABASE_EGRESS_ENGINEERING_RULES.md`**
