...
Platform Owner assign plan
  ...
  → RouteAuthBoundary deep-link deny (+ MTG allowlist)
  → organization_feature_enabled(_for) (subscription status + feature row)
  → Portal: get_my_portal_context feature gate
  → RPC require_organization_feature / tenant_module_write_allowed
  → RLS RESTRICTIVE policies on gated write tables
```

UI hide alone is **not** security. Denied modules must fail at Route → Action → RPC → RLS.

Assignable commercial products in Platform Access are **`AVS_*` only**. Legacy `ORNEXA_*` Plan Builder rows remain for internal tooling and are not offered in the tenant Assign Plan picker.

## Gap-closure migration

- `supabase/migrations/20260827030000_platform_access_gap_closure.sql` — suspend fails closed; portal feature gate; RLS on `repairs` / `job_cards` / `invoices` / `melt_jobs`.
...
- Gap closure: `supabase/migrations/20260827030000_platform_access_gap_closure.sql`
