...
describe("entitlement-route-map", () => {
  ...
  it("MTG allowlist blocks full-ERP chrome paths", () => {
    ...
    expect(pathAllowedByEntitlements("/stock/boxes", hasAll, true, { isMtg: true })).toBe(false);
    expect(pathAllowedByEntitlements("/scheme", hasAll, true, { isMtg: true })).toBe(false);
  });
});
