...
import { renderTemplate } from "@/lib/comm/email-templates";
...
describe("MTJ ERP — Master A–Z Deep System & 2-Year Volume Performance Audit", () => {
  ...
  describe("1. Two-Year Realistic Operational Dataset & Query Performance Benchmark", () => {
    it("generates 24 months of continuous shop operating data (750+ invoices, 1200+ payments, 400+ orders, 1500+ ledger entries) and compiles ledgers in < 150ms", () => {
      ...
      expect(custLedgerTime).toBeLessThan(50); // Under 50ms
      expect(custLedger.invoices.length).toBeGreaterThan(0);
      ...
      expect(karigarBookTime).toBeLessThan(50); // Under 50ms
      ...
      expect(profitReport.grossRevenuePaise).toBe(totalSalesPaise);
    });
  });
  ...
  describe("2. Comprehensive Invoicing Test Matrix (Scenarios A through G)", () => {
    ...
    it("Scenario C: Mixed payment automatically computes exact remaining cash for 1g gold shortfall", () => {
      ...
      const shortfallFineMg = invoiceFineMg - paidGoldMg; // 1000 mg
      const expectedCashPaise = calculateGoldValuePaise(shortfallFineMg, ratePaise);
      expect(expectedCashPaise).toBe(7600000); // Exactly ₹76,000.00
    });
  ...
  describe("3. Karigar Multi-Purity Custody Matrix & Purity Segregation", () => {
    it("tracks 22K, 21K, 18K, and 14K in strictly isolated books with physical weights, allowed wastage and over-loss", () => {
      ...
      // 22K Physical Balance: 50,000g issued - 48,000g returned = 2,000mg gross (1,832 mg fine)
      expect(b22.currentBalanceMg).toBe(1832);

      // 18K Physical Balance: 30,000g issued, 0 returned = 30,000mg gross (22,500 mg fine)
  ...
  describe("4. Staff / Portal Invitation & Email Template Rendering", () => {
    it("renders authoritative invitation and order delay email templates with exact tokens", () => {
      const inviteEmail = renderTemplate("staff_invite", {
        shopName: "Maa Tara Jewellers",
        inviteUrl: "https://aurum.arivahly.in/invite/accept?token=tok_invite_12345",
        role: "Billing Cashier",
        invitedBy: "Aritra Manna",
      });
      ...
      const delayEmail = renderTemplate("order_delayed", {
        shopName: "Maa Tara Jewellers",
        customerName: "Smt. Sunita Agarwal",
        orderNumber: "ORD-2026-088",
        itemName: "22K Traditional Jhumka",
        originalDate: "2026-08-28",
        newExpectedDate: "2026-09-03",
        delayReason: "Specialist hand-setting taking extra care for perfection",
        shopPhone: "+91 98300 00000",
      });

      expect(delayEmail.subject).toContain("Order Update");
      expect(delayEmail.html).toContain("ORD-2026-088");
      expect(delayEmail.html).toContain("2026-09-03");
    });
  });
