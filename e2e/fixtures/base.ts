...
export const test = base.extend<Fixtures>({
  ...
  authedPage: async ({ page }, use, testInfo) => {
    ...
    // Ensure database is seeded dynamically if seed.json is not present (mock E2E only).
    const seedPath = path.resolve(import.meta.dirname, "../.auth/seed.json");
    if (
    ...
    const cookieAccept = page.getByRole("button", { name: /accept all|essential only/i });
    if (await cookieAccept.first().isVisible({ timeout: 2_000 }).catch(() => false)) {
