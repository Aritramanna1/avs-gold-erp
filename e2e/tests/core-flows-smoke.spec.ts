...
async function createRealInvite(authedPage: any, email: string) {
  ...
  return {
    code: url.searchParams.get("invite") || url.searchParams.get("code") || "",
...
test.describe("Core module smoke test (signed out -> disposable account)", () => {
  ...
  test("accept a disposable staff invite, then load every core module route without crashing", async ({
    ...
    const targetUrl = new URL(invite.url).pathname + new URL(invite.url).search;
    await page.goto(targetUrl);
    await expect(page.locator("#invite-name")).toBeVisible({ timeout: 25_000 });
    ...
    const reviewBtn = page.getByTestId("legal-open-review");
    if (await reviewBtn.isVisible()) {
      await reviewBtn.click();
      const acceptLegalBtn = page.getByRole("button", { name: /accept & continue/i });
      await expect(acceptLegalBtn).toBeVisible({ timeout: 10_000 });
      await page.evaluate(() => {
        const scroller = document.querySelector('[data-testid="legal-scroll-body"]');
        if (scroller) {
          scroller.scrollTop = scroller.scrollHeight;
          scroller.dispatchEvent(new Event("scroll"));
        }
      });
      await acceptLegalBtn.click();
    }
