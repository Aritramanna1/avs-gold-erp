-- Compatibility table for the current attachment repository.
-- This intentionally does not replay the legacy permissive attachments migration.
...
create policy attachments_tenant_insert on public.attachments
  ...
  with check (
    ...
    and public.my_role()::text in ('owner','admin','ceo','manager','accountant','billing','vault','workshop')
  );
