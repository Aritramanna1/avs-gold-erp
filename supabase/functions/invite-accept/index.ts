...
Deno.serve(async (req) => {
  ...
  const action = String(body.mode ?? "validate");
  const { data: rateOk, error: rateErr } = await admin.rpc("check_invite_accept_rate_limit", {
    p_email: email,
    p_action: action,
  });
  if (rateErr) {
    console.warn("[invite-accept] rate limit check failed:", rateErr.message);
  } else if (rateOk === false) {
    return json(
      { error: "Too many invitation attempts. Please wait a minute and try again." },
      429,
    );
  }
