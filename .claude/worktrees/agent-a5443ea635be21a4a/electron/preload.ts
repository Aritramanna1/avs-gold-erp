...
const api = {
  ...
  dialog: {
    openFile: (options?: Electron.OpenDialogOptions) => ipcRenderer.invoke(IPC.DIALOG_OPEN_FILE, options),
    saveFile: (options?: Electron.SaveDialogOptions) => ipcRenderer.invoke(IPC.DIALOG_SAVE_FILE, options),
  },
  notify: {
  ...
  hardware: {
    ...
    onEvent: (listener: (event: HardwareEvent) => void): (() => void) => {
      const handler = (_event: Electron.IpcRendererEvent, payload: HardwareEvent) => listener(payload);
      ipcRenderer.on(IPC.HARDWARE_EVENT, handler);
      return () => ipcRenderer.removeListener(IPC.HARDWARE_EVENT, handler);
    },
  },
  print: {
    listPrinters: (): Promise<Electron.PrinterInfo[]> => ipcRenderer.invoke(IPC.PRINT_LIST_PRINTERS),
    printHtml: (
      html: string,
      options?: {
        silent?: boolean;
