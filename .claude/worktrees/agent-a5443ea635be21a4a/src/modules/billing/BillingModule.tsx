...
export function BillingModule({ orderId, stockId, jobId }: BillingModuleProps) {
        ...
        curr.map((it) => {
          ...
          if (
            ...
            if (billingType === "manufacturing") {
              ...
              rawMerged.fineMg = Math.round((rawMerged.netMg * rawMerged.purity) / 1000);
            }
          }
  ...
  function handleSelectStockItem(stock: (typeof stockItems)[0]) {
    ...
    const weightGramsNum = stock.grossMg / 1000;
    const makingPerGramPaise = stock.makingChargePerGPaise ?? 45000;
    const makingChargesPaise = Math.round(weightGramsNum * makingPerGramPaise);
      ...
      import("@/lib/comm/comm-automation")
        .then(({ emitBusinessEvent }) =>
          emitBusinessEvent("gold_settlement_reminder", {
            ...
            recipient: { name: customer.fullName, phone: customer.phone, email: (customer as any)?.email },
            linkedId: customerId,
            linkedType: "invoice",
            variables: { shortfallGrams: goldShortfallGrams.toFixed(3), invoiceNo: allocatedInvoiceNo },
          }),
        )
  ...
  function buildPrefilledItem(
    ...
    if (stock) {
      ...
      const weightGramsNum2 = stock.grossMg / 1000;
      const makingPerGramPaise = stock.makingChargePerGPaise ?? 45000;
      const makingChargesPaise = Math.round(weightGramsNum2 * makingPerGramPaise);
  ...
  if (confirmedInvoice) {
    ...
    const branchId = useBillingStore.getState().currentBranchId ?? "main";
        ...
        <DialogContent className="max-w-xl rounded-2xl p-6 border-gold/20">
          {activeInspectedStock &&
            (() => {
              ...
              const weightGramsNum4 = s.grossMg / 1000;
              const currentGoldRate = useSettings.getState().goldRatePerGramPaise || 0;
              const ratePaise = currentGoldRate;

              const makingPerGramPaise = s.makingChargePerGPaise ?? 45000;
              const makingChargesPaise = Math.round(weightGramsNum4 * makingPerGramPaise);
              ...
              const previewItem: InvoiceItem = {
                ...
                fineMg: s.fineMg,
                goldRatePerGramPaise: ratePaise,
                goldValuePaise: Math.round((s.fineMg * ratePaise) / 1000),
                makingChargesPaise,
                stoneChargesPaise: 0,
                otherChargesPaise: 0,
                discountPaise: 0,
                ...
                <div className="grid md:grid-cols-[180px_1fr] gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div className="flex justify-between font-mono">
                        <span>Making Charges (₹{paiseToRupees(makingPerGramPaise)}/g gross):</span>
...
function StandardItemRow({
  ...
  const [showCharges, setShowCharges] = useState(
    () =>
      it.makingChargesPaise + it.stoneChargesPaise + it.otherChargesPaise + it.discountPaise > 0,
  );
  ...
  const totalChargesPaise = it.makingChargesPaise + it.stoneChargesPaise + it.otherChargesPaise;
  const hasCharges =
    totalChargesPaise > 0 ||
    it.discountPaise > 0 ||
