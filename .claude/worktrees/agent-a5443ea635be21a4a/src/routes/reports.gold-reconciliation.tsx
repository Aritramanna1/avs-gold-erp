...
export const Route = createFileRoute("/reports/gold-reconciliation")({
  head: () => ({ meta: [{ title: "Gold Reconciliation · MTJ ERP" }] }),
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      ...
      {latest && (
        <div className="grid sm:grid-cols-3 gap-3 mb-6">
          ...
          <div className={`rounded-2xl border p-4 ${latest.exceptionCount > 0 ? "border-destructive/40 bg-destructive/5" : "border-border bg-card"}`}>
            <div className="text-xs text-muted-foreground">Discrepancies</div>
            <div className={`text-2xl font-semibold ${latest.exceptionCount > 0 ? "text-destructive" : ""}`}>
            ...
            {history.length === 0 && (
              <tr>
                <td colSpan={5} className="p-8 text-center text-muted-foreground">
                  {loading ? "Loading…" : "No reconciliation runs yet — click Run Reconciliation Now."}
                </td>
              </tr>
            )}
            {history.map((r) => (
              <>
                <tr key={r.id} className="border-b border-border last:border-0">
                  ...
                  <td className="p-3 text-right">
                    {r.exceptionCount > 0 && (
                      <Button size="sm" variant="outline" onClick={() => setExpanded(expanded === r.id ? null : r.id)}>
                          ...
                          <tbody>
                            {r.exceptions.map((e) => (
                              <tr key={e.billId}>
                                ...
                                <td className="p-2 text-destructive font-semibold">{mgToGrams(e.discrepancyMg)}g</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
