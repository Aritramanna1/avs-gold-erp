...
export const Route = createFileRoute("/orders/import")({
  head: () => ({ meta: [{ title: "Bulk Orders Import · MTJ ERP" }] }),
...
function OrdersImportPage() {
  ...
  function validateRow(raw: Record<string, string>): { errors: string[]; customerId?: string } {
    ...
    else if (!customer) errors.push(`no customer found with phone ${raw.customerPhone} — import that customer first`);
  ...
  async function handleImport() {
    ...
    setRows([]);
    setImporting(false);
    toast.success(`Imported ${success} order(s)${failed > 0 ? `, ${failed} failed` : ""}. No confirmation messages were sent to customers.`);
  }
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
      ...
      {rows.length === 0 && !result && (
        <div className="rounded-2xl border border-border bg-card p-8 text-center text-muted-foreground">
          Download the template, fill in your orders (matching customers by phone), then upload the CSV.
        </div>
      )}
    </div>
  );
