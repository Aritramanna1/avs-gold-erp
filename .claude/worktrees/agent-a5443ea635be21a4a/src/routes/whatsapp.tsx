...
export const Route = createFileRoute("/whatsapp")({
  ...
  head: () => ({ meta: [{ title: "WhatsApp Ingestion · MTJ ERP" }] }),
...
function WhatsappPage() {
  ...
  const handleConvert = async (id: string) => {
    ...
    const purityPM = parsed.permille ?? 916;
    const estMg = Math.round((parsed.estWeightG ?? 0) * 1000);
    const fineMg = Math.round((estMg * purityPM) / 1000);
