...
import { ArrowLeft, Printer, Receipt, Trash2, FileText } from "lucide-react";
...
export const Route = createFileRoute("/billing/$id")({
  head: () => ({ meta: [{ title: "Invoice · MTJ ERP" }] }),
        ...
        <aside className="space-y-3">
          <div className="rounded-2xl border border-border bg-card p-5 sticky top-4">
            ...
            <div className="space-y-1.5 text-sm">
              ...
              {inv.gst === "gst3" ? (
                <>
                  <Row k="CGST 1.5%" v={`₹ ${paiseToRupees(inv.cgstPaise)}`} />
                  <Row k="SGST 1.5%" v={`₹ ${paiseToRupees(inv.sgstPaise)}`} />
                </>
              ) : (
            ...
            {can("billing.delete") && (
              <Button
                variant="ghost"
                className="w-full mt-4 text-destructive gap-2"
                onClick={() => {
                  if (
                    confirm(
                      "Delete this invoice? Gold sale ledger entry is not reversed automatically.",
                    )
                  ) {
