...
export const Route = createFileRoute("/reports/communication-analytics")({
  head: () => ({ meta: [{ title: "Communication Analytics · MTJ ERP" }] }),
      ...
      {analytics && (
        <>
          <div className="grid sm:grid-cols-4 gap-3 mb-6">
            ...
            <div className={`rounded-2xl border p-4 ${analytics.queue.permanentlyFailed > 0 ? "border-destructive/40 bg-destructive/5" : "border-border bg-card"}`}>
              <div className="text-xs text-muted-foreground">Permanently Failed</div>
              <div className={`text-2xl font-semibold ${analytics.queue.permanentlyFailed > 0 ? "text-destructive" : ""}`}>
            ...
            <div className="rounded-2xl border border-border bg-card p-4">
              ...
              {Object.entries(analytics.byKind).map(([kind, count]) => (
                <div key={kind} className="flex justify-between text-sm py-1">
                  <span className="text-muted-foreground">{COMM_KIND_LABELS[kind as keyof typeof COMM_KIND_LABELS] ?? kind}</span>
          ...
          <tbody>
            ...
            {history.map((e) => (
              <tr key={e.id} className="border-b border-border last:border-0">
                ...
                <td className="p-3">
                  {e.recipientLabel} <span className="text-muted-foreground">({e.recipientPhone})</span>
                </td>
