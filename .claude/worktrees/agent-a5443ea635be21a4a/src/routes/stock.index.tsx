          ...
          {movements.map((m) => {
            ...
            return (
              <tr key={m.id} className="border-t border-border">
                <Td className="text-xs text-muted-foreground">{new Date(m.ts).toLocaleString()}</Td>
  ...
  async function save() {
    try {
      ...
      await add({
        ...
        huid: huid || undefined,
        makingChargePerGPaise: makingG ? Math.round(Number(makingG) * 100) : undefined,
