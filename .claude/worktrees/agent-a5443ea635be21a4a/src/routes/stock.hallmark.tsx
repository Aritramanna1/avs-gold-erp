...
export const Route = createFileRoute("/stock/hallmark")({
  head: () => ({ meta: [{ title: "Hallmark Lifecycle · MTJ ERP" }] }),
...
function HallmarkPage() {
  ...
  const [receivingLine, setReceivingLine] = useState<{ batch: HallmarkBatch; itemId: string; itemCode: string } | null>(null);
  ...
  const branchBatches = useMemo(
    () => batches.filter((b) => b.branchId === (selectedBranchId || "MAIN")).sort((a, b) => b.sentAt - a.sentAt),
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto">
          ...
          <div key={batch.id} className="rounded-2xl border border-border bg-card p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={batch.status === "received" ? "secondary" : batch.status === "closed" ? "outline" : "default"}>
                  {batch.status.replace(/_/g, " ")}
                </Badge>
              </div>
              {batch.status === "received" && (
                <Button size="sm" variant="outline" className="gap-1" onClick={() => useHallmarkBatches.getState().close(batch.id)}>
                  <Lock className="h-3.5 w-3.5" /> Close Batch
                </Button>
              )}
            </div>
            <div className="text-sm text-muted-foreground mb-3">
              Sent to {batch.assayCenterName} on {new Date(batch.sentAt).toLocaleDateString()} · {batch.lines.length} items
            </div>
                        ...
                        {l.status === "pending" && (
                          <div className="flex gap-1 justify-end">
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => setReceivingLine({ batch, itemId: l.itemId, itemCode: l.itemCode })}
                            >
                              Receive
                            </Button>
                            <Button
                              ...
                              onClick={() => {
                                ...
                                if (reason) useHallmarkBatches.getState().rejectItem(batch.id, l.itemId, reason);
                              }}
                            >
                              Reject
                            </Button>
      ...
      <SendBatchDialog open={sending} onClose={() => setSending(false)} branchId={selectedBranchId || "MAIN"} />
      {receivingLine && (
        <ReceiveDialog
          ...
          onConfirm={async (huid) => {
            ...
            await useHallmarkBatches.getState().receiveItem(receivingLine.batch.id, receivingLine.itemId, huid, who);
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid gap-3">
          ...
          <div>
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search items" className="my-2" />
            <div className="max-h-56 overflow-y-auto rounded-lg border border-border">
              {candidates.map((i) => (
                <label key={i.id} className="flex items-center justify-between p-2 border-b border-border last:border-0 text-sm cursor-pointer">
                  <span>
              ...
              {candidates.length === 0 && <div className="p-3 text-sm text-muted-foreground">No un-hallmarked items found.</div>}
            </div>
          </div>
        </div>
        <DialogFooter>
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <div className="flex flex-col gap-1">
          <Label>HUID *</Label>
          <Input value={huid} onChange={(e) => setHuid(e.target.value)} placeholder="6-digit HUID" />
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>
            Cancel
