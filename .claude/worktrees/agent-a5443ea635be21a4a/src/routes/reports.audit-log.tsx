...
export const Route = createFileRoute("/reports/audit-log")({
  head: () => ({ meta: [{ title: "Audit & Compliance Log · MTJ ERP" }] }),
...
function AuditLogPage() {
  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (result.ok) toast.success(`Chain verified — ${result.entriesChecked} entries, no tampering detected.`);
      ...
      {verification && (
        <div
          className={`rounded-2xl border p-4 mb-4 flex items-start gap-3 ${
            verification.ok ? "border-green-600/40 bg-green-600/5" : "border-destructive/40 bg-destructive/5"
          }`}
        >
          {verification.ok ? (
