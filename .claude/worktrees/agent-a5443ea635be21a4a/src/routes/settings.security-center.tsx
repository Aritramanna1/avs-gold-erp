...
import { listRegisteredDevices, setDeviceTrust, type DeviceInfo } from "@/lib/security/device-registry";
...
export const Route = createFileRoute("/settings/security-center")({
  head: () => ({ meta: [{ title: "Security Center · MTJ ERP" }] }),
...
function SecurityCenterPage() {
  ...
  async function handleRotateKey() {
    if (!window.confirm("Rotate the local database encryption key now? This re-encrypts the entire local database in place.")) {
      return;
    }
    setRotating(true);
    try {
      ...
      toast.success(`Encryption key rotated successfully — ${result.filesReencrypted} attachment file(s) re-encrypted.`);
    } catch (e) {
          ...
          <tbody>
            ...
            {devices.map((d) => (
              <tr key={d.deviceId} className="border-b border-border last:border-0">
                ...
                <td className="p-3 text-muted-foreground">{new Date(d.firstSeenAt).toLocaleDateString()}</td>
                <td className="p-3 text-muted-foreground">{new Date(d.lastSeenAt).toLocaleString()}</td>
                <td className="p-3">
                  {d.trusted ? (
