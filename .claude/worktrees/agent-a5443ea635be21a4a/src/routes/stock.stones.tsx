...
export const Route = createFileRoute("/stock/stones")({
  head: () => ({ meta: [{ title: "Stone & Diamond Tracking · MTJ ERP" }] }),
...
function StonesPage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
            ...
            {branchStones.map((s) => (
              <tr key={s.id} className="border-b border-border last:border-0">
                ...
                <td className="p-3 text-right">
                  {s.itemId ? (
                    <Button size="sm" variant="ghost" className="gap-1" onClick={() => useStones.getState().unlinkFromItem(s.id)}>
                      <Unlink className="h-3.5 w-3.5" /> Unlink
                    </Button>
                  ) : (
                    <Button size="sm" variant="outline" className="gap-1" onClick={() => setLinkingStone(s)}>
      ...
      <AddStoneDialog open={adding} onClose={() => setAdding(false)} branchId={selectedBranchId || "MAIN"} />
      ...
      <DialogContent className="max-w-lg">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input type="number" step="0.001" value={caratWeight} onChange={(e) => setCaratWeight(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1">
            <Label>Purchase Cost (₹)</Label>
            <Input type="number" step="0.01" value={costR} onChange={(e) => setCostR(e.target.value)} />
          </div>
          <div className="flex flex-col gap-1">
            <Label>Clarity</Label>
          ...
          <div className="flex flex-col gap-1">
            ...
            <Input value={certifyingLab} onChange={(e) => setCertifyingLab(e.target.value)} placeholder="e.g. GIA, IGI" />
          </div>
        </div>
        <DialogFooter>
  ...
  return (
    <Dialog open={!!stone} onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        ...
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search item code, barcode or name" className="mb-2" />
        <div className="max-h-64 overflow-y-auto rounded-lg border border-border">
          ...
          {candidates.length === 0 && <div className="p-3 text-sm text-muted-foreground">No matching items.</div>}
        </div>
        <DialogFooter>
