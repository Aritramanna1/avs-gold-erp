...
function WorkshopPage() {
  ...
  const issueJob = useMemo(() => jobs.find((j) => j.id === issueJobId) ?? null, [jobs, issueJobId]);
  ...
  // Process board groups by current step
  const board = useMemo(() => {
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
                    ...
                    {filtered.map((j) => {
                      ...
                      return (
                        <tr key={j.id} className="border-t border-border hover:bg-muted/20">
                          ...
                          <td className="px-4 py-3 text-xs">{cur?.name ?? "—"}</td>
                          <td className="px-4 py-3 text-xs">
                            {j.expectedDelivery ? (
        ...
        <TabsContent value="board">
          {Object.keys(board).length === 0 ? (
            <EmptyState
              ...
              body="Cards in progress will appear here grouped by current step."
            />
          ) : (
              ...
              {readyJobs.map((j) => (
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground">
                      Template:{" "}
                      {(j.templateKey && PROCESS_TEMPLATES[j.templateKey]?.name) ?? "Standard"} ·
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" className="gap-1" onClick={() => setIssueJobId(j.id)}>
      ...
      <IssueGoldDialog open={!!issueJob} onClose={() => setIssueJobId(null)} job={issueJob} />
      <ReceiveWorkDialog
        open={!!receiveJob}
