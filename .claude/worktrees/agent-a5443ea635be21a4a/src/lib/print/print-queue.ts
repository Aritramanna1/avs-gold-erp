...
async function generateFallbackPdf(job: PrintJobInput): Promise<{ blob: Blob; fileName: string }> {
  if (job.docType === "tag") {
    if (!job.tagData) throw new Error("tag print job is missing tagData — cannot generate PDF fallback.");
    return { blob: generateTagLabelPdf(job.tagData), fileName: `label-${job.tagData.itemCode || job.tagData.barcode}.pdf` };
  }
  ...
  await runLocal(() => {
    getDb().run(
      `INSERT INTO print_jobs (id, doc_type, title, status, attempts, last_error, pdf_file_name, created_at, completed_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);`,
      [id, docType, title, status, attempts, error ?? null, pdfFileName ?? null, new Date().toISOString(), new Date().toISOString()],
    );
  });
}
