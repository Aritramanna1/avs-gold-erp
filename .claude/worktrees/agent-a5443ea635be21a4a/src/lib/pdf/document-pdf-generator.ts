...
export function generateInvoicePdf(inv: any, firm: FirmProfile): Blob {
  ...
  // Totals — use cgstPaise/sgstPaise (billing store fields)
  ...
  if (cgst > 0) {
    totalLines.push({ label: "CGST (1.5%)", value: `₹${paiseToRs(cgst)}` });
    totalLines.push({ label: "SGST (1.5%)", value: `₹${paiseToRs(sgst)}` });
  }
