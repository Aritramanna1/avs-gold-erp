  ...
  await runLocal(() => {
    getDb().run(
      `INSERT INTO comm_queue (id, request, log_event_id, status, attempts, created_at, next_attempt_at)
       VALUES (?, ?, ?, 'pending', 0, ?, ?);`,
      [id, JSON.stringify(req), logEventId ?? null, new Date().toISOString(), new Date().toISOString()],
    );
  });
  return id;
}
 ...
 */
export async function drainCommQueue(): Promise<{ attempted: number; sent: number; stillPending: number }> {
  ...
  try {
    ...
    const due = (queryTable("comm_queue", "status = ? AND (next_attempt_at IS NULL OR next_attempt_at <= ?)", [
      "pending",
      now,
