...
async function sendReport(eventKey: "daily_summary" | "weekly_business_report" | "monthly_business_report", title: string, sinceMs: number) {
...
export function registerScheduledReportJobs(): void {
  ...
  registerJob({
    ...
    cadence: "monthly",
    run: () => sendReport("monthly_business_report", "Monthly", Date.now() - 30 * 24 * 60 * 60 * 1000),
  });
}
