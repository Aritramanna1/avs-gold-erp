...
function resolveEscalationRecipient(target: EscalationTier["target"]): { name: string; email?: string; phone?: string } | null {
...
function getState(entityType: string, entityId: string): Record<string, unknown> | null {
  const rows = queryTable("escalation_state", "entity_type = ? AND entity_id = ?", [entityType, entityId]);
...
export async function escalate(
  ...
  const recipient = tier.target === "responsible" ? responsibleRecipient : resolveEscalationRecipient(tier.target);
...
export async function resolveEscalation(entityType: string, entityId: string): Promise<void> {
  await initLocalDb();
  await runLocal(() => {
    getDb().run(`DELETE FROM escalation_state WHERE entity_type = ? AND entity_id = ?;`, [entityType, entityId]);
  });
}
