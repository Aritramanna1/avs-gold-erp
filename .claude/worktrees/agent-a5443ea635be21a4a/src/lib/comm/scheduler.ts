...
export async function checkDueJobs(now: Date = new Date()): Promise<{ ran: string[]; failed: string[] }> {
  await initLocalDb();
  const ran: string[] = [];
  const failed: string[] = [];
...
export async function getJobStatuses(): Promise<
  Array<{ key: string; cadence: JobCadence; lastRunAt: string | null; lastStatus: string | null; lastError: string | null }>
> {
  await initLocalDb();
