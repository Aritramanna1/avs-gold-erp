...
export interface PrintJob {
   ...
   */
  tagData?: { itemName: string; barcode: string; grossMg: number; purity: string; itemCode?: string };
}
    ...
    switch (job.type) {
      ...
      case "tag": {
        ...
        if (job.tagData) {
          ...
          return { success: true, message: "No label printer detected — downloaded as PDF instead." };
        }
