...
function createTables(): void {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS job_process_steps (
    id TEXT PRIMARY KEY,
    job_id TEXT,
    ordinal INTEGER,
    name TEXT,
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_approval_requests_status ON approval_requests(entity_type, status);`);
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_file_versions_attachment ON file_versions(attachment_id);`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_file_versions_current ON file_versions(attachment_id, is_current);`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_file_versions_entity ON file_versions(entity_type, entity_id);`);
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_gold_recon_generated ON gold_reconciliation_reports(generated_at);`);
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_financial_lock_periods_branch_period ON financial_lock_periods(branch_id, period);`);
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_physical_stock_counts_branch_id ON physical_stock_counts(branch_id);`);
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_hallmark_batches_branch_id ON hallmark_batches(branch_id);`);
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_job_process_steps_job_id ON job_process_steps(job_id);`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_stock_movements_item_id ON stock_movements(item_id);`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_worker_transactions_worker_id ON worker_transactions(worker_id);`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_worker_settlements_worker_id ON worker_settlements(worker_id);`);
  ...
  db.run(`CREATE INDEX IF NOT EXISTS idx_rate_cut_records_karigar_id ON rate_cut_records(karigar_id);`);
  db.run(`CREATE INDEX IF NOT EXISTS idx_attachments_linked ON attachments(linked_table, linked_id);`);
}
...
export async function runLocal<T>(fn: () => T | Promise<T>): Promise<T> {
  await initLocalDb();
  const database = getDb();
  database.run("BEGIN IMMEDIATE;");
  try {
...
export function isRowSoftDeleted(table: string, id: string): boolean {
  ...
  const stmt = database.prepare(
    `SELECT 1 FROM deleted_rows WHERE table_name = ? AND row_id = ?;`,
  );
  stmt.bind([table, id]);
...
export function queryTable<T = any>(table: string, whereClause = "", params: any[] = []): T[] {
  const database = getDb();
  const sql = whereClause ? `SELECT * FROM ${table} WHERE ${whereClause}` : `SELECT * FROM ${table}`;
...
export async function encryptData(key: CryptoKey, data: Uint8Array): Promise<{ blob: Uint8Array; iv: Uint8Array }> {
...
export async function decryptData(key: CryptoKey, blob: Uint8Array, iv: Uint8Array): Promise<Uint8Array> {
...
export async function rotateMasterKey(): Promise<KeyRotationResult> {
  ...
  const dbPlaintext = dbRecord ? await decryptData(oldKey, dbRecord.encryptedBlob, dbRecord.iv) : null;
  ...
  for (const { checksum, plaintext } of decryptedBlobs) {
    ...
    await runLocal(() => {
      getDb().run(`UPDATE file_blobs SET encrypted_blob = ?, iv = ? WHERE checksum = ?;`, [blob, iv, checksum]);
    });
  }
 ...
 */
export async function verifyBackupRestorable(snapshot: BackupSnapshot): Promise<BackupVerificationResult> {
  const issues: string[] = [];
  ...
  return { ok: checksumVerified && integrityCheckPassed, checksumVerified, integrityCheckPassed, sizeBytes, issues };
}
