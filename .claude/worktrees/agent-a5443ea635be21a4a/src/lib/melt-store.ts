...
const meltJobRepository = createRepository<{ id: string } & Record<string, unknown>>(
  "melt_jobs",
);
...
export function calcMeltJob(
  ...
  | "recoveryPct"
> {
  const scrapInputFineMg = Math.round((job.scrapInputGrossMg * job.scrapInputPurity) / 1000);
  const dustInputFineMg = Math.round((job.dustInputGrossMg * job.dustInputPurity) / 1000);
  const otherInputFineMg = Math.round((job.otherInputGrossMg * job.otherInputPurity) / 1000);
...
/** Generate job number: MELT-YYYYMM-NNN (sequential within existing jobs for the month) */
function generateJobNo(existingJobs: MeltJob[]): string {
  const d = new Date();
  const monthKey = `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}`;
  const prefix = `MELT-${monthKey}-`;
  const monthJobs = existingJobs.filter((j) => j.jobNo.startsWith(prefix));
  const maxSeq = monthJobs.reduce((max, j) => {
    const seq = parseInt(j.jobNo.slice(prefix.length), 10);
...
export const useMeltStore = create<MeltState>()((set, get) => ({
  ...
  createJob: async (jobData) => {
    ...
    const jobNo = generateJobNo(get().jobs);
