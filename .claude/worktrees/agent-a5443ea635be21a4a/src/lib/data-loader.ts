...
export async function pullJobSteps(): Promise<void> {
  const { error } = await supabase
    .from("job_process_steps")
    .select("id", { count: "exact", head: true });
  if (error) throw new Error(`job_process_steps pull: ${error.message}`);
}
...
export async function pullAttachments(): Promise<void> {
  ...
  useAttachments.setState({ items: dict });
...
export async function pullBackground(): Promise<{ ok: boolean; errors: string[] }> {
  ...
  await Promise.all([
    ...
    runSafe("job_process_steps", pullJobSteps, errors),
