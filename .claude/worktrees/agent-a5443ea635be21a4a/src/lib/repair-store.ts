...
function makeRepairNo(kind: RepairKind, existing: Repair[]): string {
  const d = new Date();
  const ymd = `${d.getFullYear()}${String(d.getMonth() + 1).padStart(2, "0")}${String(d.getDate()).padStart(2, "0")}`;
  const head = kind === "polishing" ? "POL" : "RPR";
  const prefix = `${head}-${ymd}-`;
  const today = existing.filter((r) => r.repairNo.startsWith(prefix));
...
export const useRepairs = create<RepairsState>()((set, get) => ({
  ...
  add: (input) => {
    ...
    const repair: Repair = {
      ...
      repairNo: input.repairNo ?? makeRepairNo(input.kind, get().repairs),
      createdAt: now,
      updatedAt: now,
  ...
  remove: (id) => {
    ...
    void repairRepository.delete(id);
  },
  reset: () => set({ repairs: [] }),
}));
