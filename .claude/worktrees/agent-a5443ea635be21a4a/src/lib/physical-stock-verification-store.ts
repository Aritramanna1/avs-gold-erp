...
interface PhysicalStockCountState {
  ...
  start: (branchId: string, actor: { id: string | null; email: string | null }) => PhysicalStockCount;
...
export const usePhysicalStockCounts = create<PhysicalStockCountState>()((set, get) => ({
  ...
  scan: (countId, barcodeOrCode) => {
    ...
    if (!line) {
      return { ok: false, message: `"${barcodeOrCode}" is not on the expected list for this branch.` };
    }
    if (line.scanned) {
