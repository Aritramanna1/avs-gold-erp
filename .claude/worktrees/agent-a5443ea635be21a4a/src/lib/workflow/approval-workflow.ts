...
async function decide(
  ...
  if (request.status !== "pending") {
    throw new Error(`Approval request ${requestId} has already been ${request.status} — cannot decide twice.`);
  }
...
export async function getApprovalHistory(entityType: string, entityId: string): Promise<ApprovalRequest[]> {
  const all = await fetchAllRequests();
  return all
