...
export function reconcileBill(bill: ManufacturingBill, toleranceMg = DEFAULT_TOLERANCE_MG): BillReconciliation {
  ...
  await runLocal(() => {
    getDb().run(
      `INSERT INTO gold_reconciliation_reports (id, generated_at, branch_id, total_checked, exception_count, report_json)
       VALUES (?, ?, ?, ?, ?, ?);`,
      [report.id, report.generatedAt, branchId ?? null, report.totalChecked, report.exceptionCount, JSON.stringify(report)],
    );
  });
