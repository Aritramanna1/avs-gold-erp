...
export async function registerThisDevice(label?: string): Promise<DeviceInfo> {
  ...
  const existing = (queryTable("device_registry", "device_id = ?", [deviceId]) as Record<
    string,
    unknown
  >[])[0];
  ...
  if (existing) {
    await runLocal(() => {
      getDb().run(`UPDATE device_registry SET last_seen_at = ? WHERE device_id = ?;`, [now, deviceId]);
    });
    return {
      deviceId,
  ...
  return { deviceId, label: resolvedLabel, platform, firstSeenAt: now, lastSeenAt: now, trusted: true };
}
...
export async function setDeviceTrust(deviceId: string, trusted: boolean): Promise<void> {
  await runLocal(() => {
    getDb().run(`UPDATE device_registry SET trusted = ? WHERE device_id = ?;`, [trusted ? 1 : 0, deviceId]);
  });
}
