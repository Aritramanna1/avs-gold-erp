 ...
 */
import { runLocal, getDb, queryTable, sha256Hex, getOrCreateSigningKey, initLocalDb } from "@/lib/local-db";
...
export async function verifyAuditChain(): Promise<ChainVerificationResult> {
  ...
  for (const row of rows) {
    ...
    if (prevHash !== expectedPrevHash) {
      issues.push(`seq ${seq}: prev_hash does not match the preceding entry's hash — chain broken.`);
    ...
    if (recomputedHash !== storedHash) {
      issues.push(`seq ${seq}: stored hash does not match recomputed hash — entry content was altered.`);
      if (brokenAtSeq === null) brokenAtSeq = seq;
    }
    ...
    if (!signatureValid) {
      issues.push(`seq ${seq}: HMAC signature invalid — entry hash was rewritten without the device signing key.`);
      if (brokenAtSeq === null) brokenAtSeq = seq;
    }
