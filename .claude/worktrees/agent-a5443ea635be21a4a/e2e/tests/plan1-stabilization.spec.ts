...
test.describe("Enterprise Security (Step 8)", () => {
    ...
    await page.evaluate(
      (id) =>
        window.__localDb.runLocal(() => {
          window.__localDb.getDb().run(`UPDATE audit_log SET after_json = ? WHERE id = ?;`, [
  ...
  test("device registration is stable and trust can be revoked", async ({ authedPage: page }) => {
    ...
    const registered = await page.evaluate(() => window.__deviceRegistry.registerThisDevice("E2E Runner"));
  ...
  test("session lock overlay blocks the app until unlocked", async ({ authedPage: page }) => {
    ...
    expect(await page.evaluate(() => window.__sessionLock.useSessionLock.getState().locked)).toBe(false);
...
test.describe("Communication Engine (Step 9)", () => {
  test("failed sends are durably queued, retried with backoff, and eventually delivered", async ({
    ...
    await page.waitForFunction(() => !!window.__commQueue && !!window.__commService, { timeout: 15000 });
    ...
    const afterFail = await page.evaluate(
      (jobId) => window.__commQueue.getQueueEntries().then((rows: any[]) => rows.find((r: any) => r.id === jobId)),
      id,
    );
  ...
  test("automation settings toggle gates event-driven sends", async ({ authedPage: page }) => {
    ...
    await page.evaluate(() =>
      window.__automationSettings.useAutomationSettings.getState().setRule("invoice_created", { enabled: false }),
    );
    await page.evaluate(() =>
...
test.describe("Print Job Queue (Priority 5)", () => {
  test("printing without a physical printer always falls back to a downloaded PDF", async ({
    ...
    expect(history.some((h: any) => h.id === result.jobId && h.status === "pdf_fallback")).toBe(true);
  });
});

test.describe("Gold Reconciliation Engine (Priority 6)", () => {
  test("flags a real fine-gold discrepancy and ties it to the audit log", async ({ authedPage: page }) => {
    await page.waitForFunction(() => !!window.__goldRecon && !!window.__auditLog, { timeout: 15000 });
    ...
    await page.evaluate(async (bill) => {
      const modPath = "/src/lib/manufacturing-bill-store.ts";
      const mod = await import(/* @vite-ignore */ modPath);
    ...
    const auditEntries = await page.evaluate(() =>
      window.__auditLog.getAuditEntries({ entityType: "manufacturing_bills", entityId: "e2e-bill-exception" }),
    );
...
test.describe("Keyboard-first workflow (Priority 4)", () => {
  test("Ctrl+F/Ctrl+K opens the global command palette; Esc closes it", async ({ authedPage: page }) => {
...
test.describe("Hardware manual fallback (Priority 7)", () => {
  test("manual barcode entry and scale-absent detection behave correctly", async ({ authedPage: page }) => {
