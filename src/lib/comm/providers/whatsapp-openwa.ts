...
export class WhatsAppOpenWaProvider implements CommProvider {
  ...
  async send(req: CommRequest, content: ResolvedContent): Promise<CommResult> {
    ...
    try {
      ...
      if (content.pdfUrl) {
        const res = await fetchWithTimeout(`${apiUrl}/sendFile`, {
          method: "POST",
          headers,
          body: JSON.stringify({
            chatId,
      ...
      const res = await fetchWithTimeout(`${apiUrl}/sendText`, {
        method: "POST",
        headers,
  ...
  async checkHealth(): Promise<OpenWaHealthStatus> {
    ...
    try {
      const res = await fetchWithTimeout(`${apiUrl}/getSessionState`, {
