...
/**
 * Data-only — "prepare reminders" per the business rule: never sends
 * anything, just lists who currently has an outstanding cash balance old
 * enough to matter. `paymentReminderDays` (Settings → Workflow) is the
 * staleness threshold — a balance due since yesterday doesn't need chasing,
 * but this list is what a manager reviews before manually sending anything.
 */
export function getPendingOutstandingBalanceReminders(): PendingReminder[] {
  const people = usePeople.getState().people;
  const thresholdMs = (useWorkflowEngine.getState().config.paymentReminderDays ?? 0) * 86_400_000;
  const cutoff = Date.now() - thresholdMs;
  return useBilling
    .getState()
    .invoices.filter((inv) => inv.balancePaise > 0 && inv.createdAt <= cutoff)
    .map((inv) => {
