...
import { fetchMyMemberships } from "@/lib/identity/membership-service";
...
function snapshotFromMembership(
  organizationId: string,
  subscriptionStatus: string | null | undefined,
): SubscriptionAccessSnapshot | null {
...
async function membershipSubscriptionFallback(
  organizationId: string | null,
): Promise<SubscriptionAccessSnapshot | null> {
  const memberships = await fetchMyMemberships({ productId: DEFAULT_AVS_PRODUCT });
...
export const useSubscriptionAccess = create<SubscriptionAccessState>()((set, get) => ({
  ...
  async refresh(organizationId) {
    ...
    try {
      ...
      if (!isSubscriptionAccessGranted(snapshot)) {
        const fromMembership = await membershipSubscriptionFallback(organizationId ?? null);
        if (fromMembership && isSubscriptionAccessGranted(fromMembership)) {
          set({ ...fromMembership, checking: false });
