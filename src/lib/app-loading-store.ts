...
interface AppLoadingState {
  ...
  markCriticalLoadDegraded: (message: string) => void;
  retryBoot: () => void;
}
...
export const useAppLoading = create<AppLoadingState>()((set) => ({
  ...
  /** Shell opens, but settings may be incomplete — show banner, not infinite spinner. */
  markCriticalLoadDegraded: (message: string) =>
    set({
      criticalLoadDone: true,
      criticalLoadFailed: true,
      criticalLoadError: message,
    }),
  retryBoot: () =>
    set((s) => ({
...
export function markInitialLoadDone(): void {
  ...
  void import("@/lib/monitoring/supabase-egress-monitor").then((m) => m.markLoginBootComplete());
}
...
export function markCriticalLoadDegraded(message: string): void {
  useAppLoading.getState().markCriticalLoadDegraded(message);
}
