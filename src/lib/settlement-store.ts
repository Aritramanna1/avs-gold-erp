...
export interface Settlement {
  ...
  finalisedAt?: number;
  /** Set once completeFinalSettlement() posts a Gold Settlement Voucher — only when the settlement included a gold_exchange payment. */
...
async function finaliseNow(
  ...
  if (s.targetInvoiceId) {
    // Settling an existing unpaid invoice — append payments to it
    const existingInv = useBilling.getState().invoices.find((i) => i.id === s.targetInvoiceId);
    if (!existingInv) throw new Error("Target invoice not found");

    // Add all payments from this settlement to the target invoice
    for (const p of s.payments) {
      await useBilling.getState().addPayment(s.targetInvoiceId, p);
    }

    // Refresh to get updated totals after payments
    const updatedInv = useBilling.getState().invoices.find((i) => i.id === s.targetInvoiceId)!;
    invoice = updatedInv;

    financialStatus = computeFinancialStatus(
      true,
      updatedInv.balancePaise,
      updatedInv.paidPaise,
      s.deliveryStatus,
    );
  } else {
    const totals = computeInvoiceTotals(s.items, s.gst, undefined, s.payments);
    invoice = await useBilling.getState().add({
      invoiceNo: undefined as unknown as string,
  ...
  // Gold Settlement Voucher — reuses the same party-facing voucher accounting
  // useGoldSettlement already posts for every other gold-in/gold-out flow
  // (Delivery Challan, BillingModule's Pay-In-Gold surplus, Outside Work).
  // This is a printable voucher record, distinct from the Gold Vault ledger
  // useBilling.addPayment() already posts to above — not a duplicate source
  // of gold quantity, the same dual-write pattern BillingModule uses.
  let goldSettlementId: string | undefined;
  const goldPayments = s.payments.filter(
    (p) => p.mode === "gold_exchange" && (p.goldFineMg ?? 0) > 0,
  );
  if (goldPayments.length > 0) {
    const grossMg = goldPayments.reduce((sum, p) => sum + (p.goldGrossMg ?? 0), 0);
    const fineMg = goldPayments.reduce((sum, p) => sum + (p.goldFineMg ?? 0), 0);
    const purity = goldPayments[0]?.goldPurity ?? 916;
    try {
      const voucher = await useGoldSettlement.getState().addSettlement({
        party_type: "customer",
        ...
        amount_paise: 0,
        notes: `Gold Settlement Voucher — Final Settlement ${s.settlementNo} / Invoice ${invoice.invoiceNo}`,
        payment_mode: "gold_exchange",
        direction: "Jama",
      });
      goldSettlementId = voucher.id;
    } catch (err) {
      console.error("[SettlementStore] Failed to post Gold Settlement Voucher:", err);
    }
  }
