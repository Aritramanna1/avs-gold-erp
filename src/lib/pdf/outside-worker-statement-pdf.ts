...
export async function generateOutsideWorkerStatementPdf(
  ...
  doc.text(firm.shopName || "Jewellers ERP", textStartX, y);
  doc.setFontSize(12);
  ...
  doc.text(`Outside Jeweller: ${data.jewellerName}`, textStartX, y);
  doc.text(`Period: ${data.periodLabel}`, COL_R, y, { align: "right" });
  y += 4;
  doc.text(`Generated: ${fmtDate(Date.now())}`, textStartX, y);
  y = hrule(doc, y + 3);
