...
export function fineMetalMg(weightMg: number, purityPermille: number): number {
  if (!Number.isSafeInteger(weightMg) || weightMg < 0) throw new Error("Weight must be a non-negative integer in milligrams.");
  if (!Number.isSafeInteger(purityPermille) || purityPermille <= 0 || purityPermille > 1000) throw new Error("Purity must be between 1 and 1000 per-mille.");
...
export function calculateComposition(input: {
  ...
  return {
    ...
    inputFineMg,
    alloyTotalMg,
    expectedOutputMg: Math.max(0, targetFineMg - Math.round((targetFineMg * input.formula.expectedLossPct) / 100)),
    components,
  };
}

export function calculateLoss(inputWeightMg: number, actualOutputMg: number, recoveryMg = 0) {
  if (inputWeightMg < 0 || actualOutputMg < 0 || recoveryMg < 0) throw new Error("Loss and recovery values cannot be negative.");
