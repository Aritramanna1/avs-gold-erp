...
type State = {
  ...
  saveWithFile: (
    ...
    docKey: string,
    file: File,
    patch?: { filed?: boolean; note?: string; uploadedBy?: string; portalType?: PortalType },
...
export const useAttachments = create<State>()((set, getStore) => ({
  ...
  saveWithFile: async (t, id, k, file, patch = {}) => {
    ...
    if (mimeType.startsWith("image/")) {
      try {
        // List thumbs only — full design stays near-original in R2.
        const thumbEdge = t === "catalog" ? 480 : 240;
        thumbnailDataUrl = await generateImageThumbnail(file, thumbEdge);
      } catch (err) {
...
export async function copyAttachment(
  ...
  if (src.storagePath && src.bucket) {
    try {
      const url = await getAttachmentSignedUrl(src.bucket, src.storagePath);
      const res = await fetch(url);
      if (!res.ok) return false;
      const blob = await res.blob();
      const file = new File([blob], src.fileName || "copy.bin", {
        type: src.mimeType || blob.type || "application/octet-stream",
      });
      await useAttachments.getState().saveWithFile(to.entityType, to.entityId, to.docKey, file, {
        filed: true,
        note: src.note,
      });
      return true;
...
export function invalidateAttachmentUrl(
  ...
  const rec = useAttachments.getState().items[key];
  if (rec?.bucket && rec.storagePath) {
    void import("@/lib/supabase-storage").then(({ revokeR2DisplayUrl }) => {
      revokeR2DisplayUrl(rec.bucket!, rec.storagePath!);
    });
  }
}
