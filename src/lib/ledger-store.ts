...
import type { FormulaSnapshot } from "./gold-calculation-rules";
...
export interface LedgerEntry {
  ...
  /** Precious metal scope — defaults to gold for backward compatibility. */
  metalCode?: string;
  type: MovementType;
  ...
  reference?: string;
  /** Trace chain — module that posted this entry. */
  source?: string;
  sourceId?: string;
...
const LEDGER_COMPAT_CACHE_LIMIT = 150;

export const useLedger = create<LedgerState>()((set, get) => ({
  entries: [],
  refresh: async () => {
    // Keep a bounded in-memory cache for balance math — full history belongs in
    // report RPC pagination (fetchGoldLedgerPage), not a startup-style hydrate.
    const page = await fetchGoldLedgerPage({
      limit: LEDGER_COMPAT_CACHE_LIMIT,
      offset: 0,
      order: "desc",
    });
    set({ entries: page.rows.sort((a, b) => a.createdAt - b.createdAt) });
  },
  append: async (input) => {
    ...
    if (useWorkflowEngine.getState().config.financialLockEnforcementEnabled) {
      await assertPeriodOpenOnline(
        resolveOperationalBranchId(useSettings.getState().selectedBranchId),
    ...
    if (
      entry.type !== "sale" &&
      entry.type !== "job_work_delivery" &&
      entry.type !== "reversal" &&
      entry.type !== "workshop_process_loss" &&
      entry.type !== "customer_gold_credit_applied"
    ) {
      void import("@/lib/ui-feedback").then(({ emitUiFeedback }) => emitUiFeedback("gold"));
    }
    return entry;
  },
...
export function computeBalances(entries: LedgerEntry[], metalCode = "gold"): BalanceSheet {
  const scoped = entries.filter(
    (e) => (e.metalCode ?? "gold").toLowerCase() === metalCode.toLowerCase(),
  );
...
const ASSISTANT_LEDGER_PAGE = 1000;

/**
 * Full-firm ledger balances for Assistant / reports — does not use the
 * truncated Zustand cache (`LEDGER_COMPAT_CACHE_LIMIT`). Prefers RPC
 * `get_firm_ledger_balances` (no full `data` JSON download); falls back to
 * paginated `data` select when the RPC is unavailable.
 */
export async function fetchFirmLedgerBalances(options?: {
  branchId?: string | null;
  metalCode?: string;
}): Promise<{ balances: BalanceSheet; entries: LedgerEntry[] }> {
  ...
  try {
    const { data, error } = await supabase.rpc(
      "get_firm_ledger_balances" as never,
      {
        p_branch_id: bid,
        p_metal_code: options?.metalCode ?? "gold",
      } as never,
    );
    if (!error && data && typeof data === "object") {
      ...
      const buckets: BucketBalances = {
        vault: Math.round(row.buckets?.vault ?? 0),
        karigar: Math.round(row.buckets?.karigar ?? 0),
        finished: Math.round(row.buckets?.finished ?? 0),
        customer: Math.round(row.buckets?.customer ?? 0),
        jeweller: Math.round(row.buckets?.jeweller ?? 0),
        scrap: Math.round(row.buckets?.scrap ?? 0),
      };
      const emptyBreakdown = (): BucketBreakdown => ({
        fineMg: 0,
        grossMg: 0,
        purities: {},
        cashPaise: 0,
      ...
      const bucketBreakdowns: Record<Bucket, BucketBreakdown> = {
        vault: { ...emptyBreakdown(), fineMg: buckets.vault },
        karigar: { ...emptyBreakdown(), fineMg: buckets.karigar },
        finished: { ...emptyBreakdown(), fineMg: buckets.finished },
        customer: { ...emptyBreakdown(), fineMg: buckets.customer },
        jeweller: { ...emptyBreakdown(), fineMg: buckets.jeweller },
        scrap: { ...emptyBreakdown(), fineMg: buckets.scrap },
      };
      const ledgerTotal = Math.round(row.ledgerTotal ?? 0);
      const totalUnderManagement = Math.round(
        row.totalUnderManagement ??
          buckets.vault +
            buckets.karigar +
            buckets.finished +
            buckets.customer +
      ...
      const discrepancyMg = Math.round(row.discrepancyMg ?? totalUnderManagement - ledgerTotal);
      return {
        balances: {
          buckets,
          bucketBreakdowns,
          ...
          ledgerTotal,
          discrepancyMg,
          balanced: row.balanced ?? discrepancyMg === 0,
          entryCount: Math.round(row.entryCount ?? 0),
          cashBuckets: {
            vault: 0,
            karigar: 0,
            finished: 0,
    ...
    /* fall through to paginated select */
  }
  ...
  for (;;) {
    const page = await fetchGoldLedgerPage({
      limit: ASSISTANT_LEDGER_PAGE,
      offset,
    });
    entries.push(...page.rows);
    if (page.rows.length < ASSISTANT_LEDGER_PAGE) break;
    offset += ASSISTANT_LEDGER_PAGE;
  }

  const sorted = entries.sort((a, b) => a.createdAt - b.createdAt);
  return { balances: computeBalances(sorted), entries: sorted };
}
...
export const MOVEMENT_LABELS: Record<MovementType, string> = {
  ...
  credit_note_gold_relief: "Credit Note Gold Relief",
