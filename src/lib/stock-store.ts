...
export interface StockItem {
  ...
  /** Canonical item master when picked from registry. */
  itemMasterId?: string;
...
interface StockState {
  ...
  sellReadyStock: (
    id: string,
    invoiceNo: string,
    opts?: { ledgerAlreadyPosted?: boolean },
  ) => Promise<void>;
...
const STOCK_COMPAT_CACHE_LIMIT = 200;
const STOCK_MOVEMENT_COMPAT_CACHE_LIMIT = 200;
...
export const useStock = create<StockState>()((set, get) => ({
  ...
  add: async (input, opts) => {
    ...
    const fineMg = computed.fineMg;
    if (fineMg > 0 && !opts?.goldLedgerPosted) {
      throw new Error(
        "Ready/finished gold stock must use addReadyStock() so Gold Vault is posted. Direct stock.add() is blocked when fineMg > 0.",
      );
    }
  ...
  addReadyStock: async (input, source) => {
    ...
    const fineMg = computed.fineMg;
    // Receive-work already posted karigar → finished (net 0). A manufactured
    // ready-stock row must not add a second +finished movement.
    if (source !== "manufactured") {
      await useLedger.getState().append({
        type: "ready_stock_purchase_received",
        netFineMg: fineMg,
    ...
    return get().add(input, { goldLedgerPosted: true });
  },
  sellReadyStock: async (id, invoiceNo, opts) => {
    const item = get().items.find((i) => i.id === id);
    if (!item) throw new Error("Stock item not found.");
    if (item.status === "sold") throw new Error("This item is already marked sold.");
    // Invoice finalize already posts finished− via postInvoiceGoldLedgerForSale —
    // skip a second sale movement when called from billing.
    if (!opts?.ledgerAlreadyPosted) {
      const { useLedger } = await import("./ledger-store");
      await useLedger.getState().append({
        type: "sale",
      ...
      fineMg:
        patch.netMg != null || patch.purity != null
          ? computeFineGold(
              {
                module: "stock",
                grossMg: patch.netMg ?? item.netMg,
                lessMg: 0,
                purityPermille: patch.purity ?? item.purity,
              },
              currentGoldCalculationRules(),
            ).fineMg
          : item.fineMg,
  ...
  findByBarcode: (barcode) => {
    ...
    return get().items.find(
      (i) =>
        i.barcode === b ||
        i.itemCode.toLowerCase() === lower ||
        (i.huid != null && i.huid.trim().toLowerCase() === lower),
    );
  },
