/**
 * AVS ERP — Workers / Attendance / Settlement store
 *
...
export const SALARY_RULE_LABELS: Record<SalaryRuleType, string> = {
  ...
  work_based: "Work-Based (% of gold worked)",
  making_charge: "Making-Charge Based",
  wastage_basis: "Wastage-Basis Worker",
  custom: "Custom Rule",
};

/** How wage gold is settled at final payment. */
export type WagePayMode = "book_purity_gold" | "cash_at_rate" | "either";

export const WAGE_PAY_MODE_LABELS: Record<WagePayMode, string> = {
  book_purity_gold: "Pay in book purity gold",
  cash_at_rate: "Pay cash at today's rate",
  either: "Gold or cash (choose at settlement)",
};
...
export interface SalaryRule {
  ...
  wastageNotes?: string;
  /**
   * Work-based wage: percentage components of gold worked that sum to total %.
   * Example: [1.5, 1.5] → 3% of 200g = 6g before overloss (user's 1.5+1.5 example).
   */
  wagePctComponents?: number[];
  /** Book purity for wage gold (e.g. 916 for 22K ledger). */
  bookPurityPermille?: number;
  wagePayMode?: WagePayMode;
  weeklyAllowancePaise?: number;
...
/** Food / weekly / monthly pocket money — deducted or shown on salary sheet. */
export interface WorkerAllowancePayment {
  id: string;
  workerId: string;
  date: string;
  amountPaise: number;
  kind: "weekly_food" | "monthly" | "other";
  mode: PayMode;
  notes?: string;
  createdAt: number;
}
...
export interface Settlement {
  ...
  leaveDays: number;
  payableDays: number;
  /** Calendar days on-site in period (incoming → outgoing stays). */
  daysWorked?: number;
  // cash
...
interface WorkersState {
  ...
  allowances: WorkerAllowancePayment[];
  ...
  markArrived: (workerId: string, notes?: string, arrivedAt?: number) => Promise<WorkerStay>;
  markGoneHome: (workerId: string, notes?: string, departedAt?: number) => Promise<void>;
  ...
  addAllowance: (
    a: Omit<WorkerAllowancePayment, "id" | "createdAt"> & { id?: string },
  ) => Promise<WorkerAllowancePayment>;
  addGoldAdvance: (
    g: Omit<GoldAdvance, "id" | "createdAt"> & {
      id?: string;
      /** Caller already posted gold_ledger (e.g. Gold Settlement voucher). */
      skipGoldLedger?: boolean;
    },
  ) => Promise<GoldAdvance>;
  addWastageReturn: (
    w: Omit<WastageGoldReturn, "id" | "createdAt"> & {
      id?: string;
      skipGoldLedger?: boolean;
    },
...
export const useWorkers = create<WorkersState>()((set, get) => ({
  ...
  refresh: async () => {
    ...
    const TX_CACHE_LIMIT = 80;

    let attQ = withFirmScope(
      supabase
        .from("attendance")
        .select("data")
    ...
    const [attRes, rulesRes, txRes, setRes] = await Promise.all([
      attQ,
      withFirmScope(supabase.from("salary_rules").select("data").limit(200), firmId),
      txQ,
      setQ,
    ]);
    ...
    const allowances: WorkerAllowancePayment[] = [];
    ...
    (txRes.data ?? []).forEach((r) => {
      ...
      else if (r.kind === "allowance") allowances.push(payload);
  ...
  markArrived: async (workerId, notes, arrivedAt) => {
  ...
  markGoneHome: async (workerId, notes, departedAt) => {
    ...
    const closed: WorkerStay = {
      ...openStay,
      departedAt: departedAt ?? Date.now(),
  ...
  bulkUpsertAttendance: async (inputs) => {
    ...
    const now = Date.now();
    const branchId = resolveOperationalBranchId(useSettings.getState().selectedBranchId);
  ...
  addAllowance: async (input) => {
    const a: WorkerAllowancePayment = {
      id: input.id ?? makeId("al"),
      createdAt: Date.now(),
      ...input,
    };
    await workerTransactionRepository.save({ ...a, kind: "allowance" });
    await get().refresh();
    return a;
  },
  addGoldAdvance: async (input) => {
    ...
    if (!(g.fineMg > 0)) {
      throw new Error("Gold advance fine weight must be greater than zero.");
    }
    if (input.ledgerEntryId) {
      g.ledgerEntryId = input.ledgerEntryId;
    } else if (!input.skipGoldLedger) {
      const { useLedger } = await import("./ledger-store");
      const { assertVaultGoldIssueAvailable } = await import("./vault-gold-stock");
      const ledgerEntries = useLedger.getState().entries;
      assertVaultGoldIssueAvailable({
        entries: ledgerEntries,
        purityPermille: g.purity,
        fineMg: g.fineMg,
        grossMg: g.grossMg,
      });
      const ledgerEntry = await useLedger.getState().append({
        ...
        netFineMg: -g.fineMg,
        deltas: { vault: -g.fineMg },
        grossMg: g.grossMg,
        purity: g.purity as any,
        fineMg: g.fineMg,
        reference: g.id,
        notes: `Worker gold advance${g.reason ? `: ${g.reason}` : ""}${g.notes ? ` — ${g.notes}` : ""}`,
        karigarId: g.workerId,
      });
      g.ledgerEntryId = ledgerEntry.id;
    }
  ...
  addWastageReturn: async (input) => {
    ...
    if (!(w.fineMg > 0)) {
      throw new Error("Wastage return fine weight must be greater than zero.");
    }
    if (input.ledgerEntryId) {
      w.ledgerEntryId = input.ledgerEntryId;
    } else if (!input.skipGoldLedger) {
      const { useLedger } = await import("./ledger-store");
      const ledgerEntry = await useLedger.getState().append({
        type: "worker_wastage_gold_return",
        netFineMg: w.fineMg,
        deltas: { vault: w.fineMg },
        grossMg: w.grossMg,
        purity: w.purity as any,
        fineMg: w.fineMg,
        reference: w.id,
        notes: `Worker wastage gold return${w.notes ? `: ${w.notes}` : ""}`,
        karigarId: w.workerId,
      });
      w.ledgerEntryId = ledgerEntry.id;
    }
...
export function salaryEarnedPaise(
  ...
  // work_based / making_charge / wastage_basis / custom: wage gold + cash settled separately
  return 0;
}

/** Sum of configurable % components (e.g. 1.5 + 1.5 → 3). */
export function wagePctTotal(rule: SalaryRule | undefined): number {
  if (!rule?.wagePctComponents?.length) return 0;
  return rule.wagePctComponents.reduce((a, b) => a + (Number(b) || 0), 0);
}

/**
 * Gold issued (gross mg) to the worker in the period — used as “grams worked”
 * for % wage. Override at settlement when needed.
 */
export function gramsWorkedMgForWorker(
  workerId: string,
  fromDate: string,
  toDate: string,
): number {
  ...
  let sum = 0;
  for (const e of entries) {
    if (e.workerId !== workerId || e.type !== "given") continue;
    const d =
      e.date ||
    ...
    sum += e.grossMg || e.netMg || 0;
  }
  return sum;
}

/** Calendar days on-site intersecting [fromDate, toDate] from stay in/out. */
export function stayDaysInPeriod(
  ...
  return stays
    ...
    .reduce((total, stay) => {
      const start = Math.max(stay.arrivedAt, fromMs);
      const end = Math.min(stay.departedAt ?? Date.now(), toMs);
      if (end <= start) return total;
      return total + (end - start) / 86_400_000;
    }, 0);
}

/** wageGross = worked × pct/100; wageNet = max(0, gross − overloss). All in mg. */
export function computeWorkBasedWageMg(opts: {
  gramsWorkedMg: number;
  wagePct: number;
  overlossMg: number;
}): { wageGrossMg: number; wageNetMg: number } {
  const pct = Math.max(0, opts.wagePct);
  const worked = Math.max(0, Math.round(opts.gramsWorkedMg));
  const wageGrossMg = Math.round((worked * pct) / 100);
  const wageNetMg = Math.max(0, wageGrossMg - Math.max(0, Math.round(opts.overlossMg)));
  return { wageGrossMg, wageNetMg };
}

/** Cash equivalent of wage gold at book-purity rate (paise per gram). */
export function wageGoldCashPaise(wageNetMg: number, ratePerGramPaise: number): number {
  if (wageNetMg <= 0 || ratePerGramPaise <= 0) return 0;
  return Math.round((wageNetMg * ratePerGramPaise) / 1000);
}

/** monthly_only workers never settle wastage. */
export function workerAllowsWastage(
  mode: "wastage_only" | "monthly_only" | "monthly_and_wastage" | undefined,
): boolean {
  return mode !== "monthly_only";
}

export const COMPENSATION_MODE_LABELS: Record<
  "wastage_only" | "monthly_only" | "monthly_and_wastage",
  string
> = {
  wastage_only: "Wastage worker only (no fixed salary)",
  monthly_only: "Salary / allowance only (no wastage)",
  monthly_and_wastage: "Monthly pay + wastage",
};

export const ALLOWANCE_KIND_LABELS: Record<WorkerAllowancePayment["kind"], string> = {
  weekly_food: "Weekly food / pocket",
...
export function buildWorkerPassbookRows(
  ...
  for (const a of (data.allowances ?? []).filter((x) => x.workerId === workerId))
    rows.push({
      date: a.date,
      label: ALLOWANCE_KIND_LABELS[a.kind] ?? "Allowance",
      cashIn: a.amountPaise,
      note: a.notes,
    });
  ...
  for (const s of data.settlements.filter((x) => x.workerId === workerId)) {
    rows.push({
      ...
      note: `${s.daysWorked ?? s.payableDays} days worked`,
    });
    if ((s.wageNetMg ?? 0) > 0) {
      rows.push({
        date: s.toDate,
        label:
          s.wagePayOutMode === "cash"
            ? "Wage gold paid as cash"
            : "Wage gold owed (book purity)",
        cashIn: s.wagePayOutMode === "cash" ? s.wageCashPaise : undefined,
        goldOut: s.wagePayOutMode !== "cash" ? s.wageNetMg : undefined,
        note:
          s.wagePayOutMode === "cash"
            ? `${mgToGrams(s.wageNetMg ?? 0)} g @ rate → ${paiseToRupees(s.wageCashPaise ?? 0)}`
            : `${mgToGrams(s.wageNetMg ?? 0)} g @ ${s.wageBookPurity ?? "—"}‰`,
      });
    }
