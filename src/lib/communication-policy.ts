/**
 * Platform communication policy (WhatsApp API / legacy PHP / credits).
 * Recovered from production-dist-shop/assets/communication-policy-*.js
 */
...
let cached: CommunicationPolicy | null = null;
...
export function getCachedCommunicationPolicy(): CommunicationPolicy {
  return cached ?? { ...DEFAULT_COMMUNICATION_POLICY };
}
...
export async function loadCommunicationPolicy(force = false): Promise<CommunicationPolicy> {
  ...
  inflight = (async () => {
    try {
      const { data, error } = await (supabase as any).rpc("communication_policy");
      ...
      /* fall through to platform_settings */
    }
    ...
    if (error) {
      ...
      cached = { ...DEFAULT_COMMUNICATION_POLICY };
      return cached;
    }

    const map = new Map((data ?? []).map((r: { key: string; value: unknown }) => [r.key, r.value]));
    cached = {
      whatsapp_api_enabled: map.has(COMMUNICATION_POLICY_KEYS.whatsappApi)
...
export function isLegacyPhpWhatsAppProvider(providerId: string | null | undefined): boolean {
  return LEGACY_PHP_PROVIDER_RE.test(providerId ?? "");
}

export function canSendOfficialWhatsAppApi(policy?: CommunicationPolicy | null): boolean {
  return (policy ?? getCachedCommunicationPolicy()).whatsapp_api_enabled;
}

export function canSendLegacyPhpWhatsApp(policy?: CommunicationPolicy | null): boolean {
  return (policy ?? getCachedCommunicationPolicy()).legacy_whatsapp_php_enabled;
}

export function canDeductWhatsAppCredits(policy?: CommunicationPolicy | null): boolean {
  const p = policy ?? getCachedCommunicationPolicy();
  return p.whatsapp_api_enabled && p.whatsapp_credits_enabled;
}

export function canDeductAiCredits(policy?: CommunicationPolicy | null): boolean {
  return (policy ?? getCachedCommunicationPolicy()).ai_credits_enabled;
}

export function whatsappApiBlockedReason(policy?: CommunicationPolicy | null): string | null {
  return canSendOfficialWhatsAppApi(policy)
    ? null
    : "Official WhatsApp API is turned off. Automatic messages go by Email. Use Share to send this document from WhatsApp on this device.";
}

export function legacyPhpBlockedReason(policy?: CommunicationPolicy | null): string | null {
  return canSendLegacyPhpWhatsApp(policy)
    ? null
    : "Legacy PHP / custom WhatsApp sending is turned off by Platform Owner.";
}

export async function maybeDeductWhatsAppCredits(input: {
  serviceCode?: string;
  units?: number;
  referenceId?: string | null;
  description?: string | null;
}): Promise<{ success: boolean; skipped?: boolean; error?: string }> {
  if (!canDeductWhatsAppCredits(await loadCommunicationPolicy())) {
    return { success: false, skipped: true };
  }
  const { error, data } = await (supabase as any).rpc("deduct_tenant_credits", {
    p_service_code: input.serviceCode ?? "wa_utility",
  ...
  if (data && (data as { success?: boolean }).success === false) {
    return {
      success: false,
      skipped: true,
      error: String((data as { error?: string }).error ?? "credit_denied"),
    };
  }
  return { success: true };
}
