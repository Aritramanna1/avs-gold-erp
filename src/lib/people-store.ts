...
export interface Person {
  ...
  compensationMode?: "wastage_only" | "monthly_only" | "monthly_and_wastage";
...
export const usePeople = create<PeopleState>()((set, get) => ({
  ...
  refresh: async () => {
    ...
    const { rowToPerson } = await import("@/lib/people-query");
    ...
    const rows = (data ?? [])
      .map((r) => rowToPerson(r as Parameters<typeof rowToPerson>[0]))
