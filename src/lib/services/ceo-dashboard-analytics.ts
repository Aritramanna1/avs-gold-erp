...
export async function fetchCeoManufacturingGoldSummary(): Promise<CeoManufacturingGoldSummary> {
  ...
  const workerOutstandingMg = await fetchWorkerOutstandingMg();
...
export async function fetchCeoProductionPipeline(): Promise<CeoProductionPipelinePoint[]> {
  ...
  await Promise.all(
    JOB_STATUS_FLOW.map(async (status) => {
      ...
      const counts = await Promise.all(sourceStatuses.map((s) => countJobCardsByStatus(s)));
      statusCounts.set(
        status,
...
async function countJobCardsByStatus(status: string): Promise<number> {
...
async function fetchWorkerOutstandingMg(): Promise<number> {
  const balances = await fetchWorkerPendingGoldMap();
  return Array.from(balances.values()).reduce((sum, mg) => sum + Math.abs(mg), 0);
}

async function fetchWorkerPendingGoldMap(): Promise<Map<string, number>> {
  const { data, error } = await (supabase as any)
