...
function branchFilter(query: any, branchId: string, firmId: string | null) {
  let scoped = query.or(`branch_id.eq.${branchId},data->>branchId.eq.${branchId}`);
  if (firmId) scoped = scoped.eq("firm_id", firmId);
  return scoped;
}
...
async function fallbackBranchKPIs(branchId: string): Promise<BranchKPIs> {
  ...
  const [
    ...
    count("repairs", branchId, firmId, (query) =>
      query
        .not("status", "in", "(delivered,cancelled)")
        .not("data->>status", "in", "(delivered,cancelled)"),
    ),
    count("people", branchId, firmId, (query) => query.or("type.eq.customer,data->>type.eq.customer")),
  ]);
