...
async function fetchPeopleByTypes(types: Person["type"][]): Promise<Person[]> {
  ...
  return usePeople
    .getState()
    .people.filter((p) => types.includes(p.type))
...
async function fetchGoldSettlements(partyTypes: GoldSettlementRecord["party_type"][]) {
  ...
  return (useGoldSettlement.getState().settlements || []) as unknown as GoldSettlementRecord[];
}
...
export async function fetchWorkerReportRows(): Promise<{
  ...
  let entries: WorkerGoldBookEntry[] = [];
  try {
    ...
    if (!transactionRows.error && transactionRows.data) {
      entries = ((transactionRows.data ?? []) as unknown[])
  ...
  if (entries.length === 0) {
    entries = useWorkerGoldBook.getState().entries;
  }

  const [workers, settlements] = await Promise.all([
  ...
  return {
    ...
    capped:
      ...
      entries.length >= PARTY_REPORT_LIMIT,
  };
}
