...
function rowToPerson(row: PeopleRow): Person | null {
  const data = row.data ?? {};
  ...
  return {
    ...
    maxFineGoldCreditMg: data.maxFineGoldCreditMg,
...
export async function fetchPeopleTabCounts(branchId?: string | null): Promise<PeopleTabCounts> {
  const [customers, firms, karigars, employees, vendors] = await Promise.all([
    countPeople(["customer"], branchId),
    countPeople(["firm_customer"], branchId),
    countPeople(["karigar", "worker"], branchId),
    countPeople(["employee"], branchId),
    countPeople(["vendor", "outside_worker"], branchId),
  ]);
