...
export async function hydrateCustomerLedgerContext(
  ...
  const [invoiceRes, orderRes, settlementRes, jobRes, challanRes] = await Promise.all([
    db.from("invoices").select("data").eq("customer_id", customerId).limit(1000),
    db.from("orders").select("data").eq("customer_id", customerId).limit(1000),
    db
      ...
      .limit(1000),
    db.from("job_cards").select("data").eq("customer_id", customerId).limit(1000),
    db.from("delivery_challans").select("data").eq("customer_id", customerId).limit(1000),
  ]);
