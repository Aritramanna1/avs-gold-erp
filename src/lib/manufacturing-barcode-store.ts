...
import { verifyBarcodeUrl } from "@/lib/link-hosts";
...
const MANUFACTURING_BARCODE_CACHE_LIMIT = 300;
...
export const useManufacturingBarcodes = create<ManufacturingBarcodeState>()((set, get) => ({
  ...
  refresh: async () => {
    ...
    const { data, error } = await supabase
      .from("manufacturing_barcodes" as any)
      .select("data")
      .eq("firm_id", firmId)
      .order("updated_at", { ascending: false })
      .limit(MANUFACTURING_BARCODE_CACHE_LIMIT);
    if (error) {
  ...
  generate: async (order, customerName, input, actor) => {
    ...
    try {
      ...
      const qrPayload =
        config.qrFormat === "url"
          ? verifyBarcodeUrl(barcodeNumber)
          : JSON.stringify({
