                ...
                {partyId && kind && (
                  <Button
                    ...
                    onClick={async () => {
                      ...
                      const printUrl = `/workshop/gold-book-print/${partyId}?kind=${kind}&purity=${pb.purity}&from=${period.range.from}&to=${period.range.to}&plabel=${encodeURIComponent(period.range.label)}`;
                      usePrintEngine.getState().triggerPrint(printUrl, `${partyName ?? "Artisan"} · ${pb.purity} Book Statement`);
                    }}
                  >
