...
import { isCustomerParty, isKarigarParty, isSupplierParty } from "@/lib/party-types";
...
import { cashBookEnabled } from "@/lib/invoice-due";
...
export interface SettlementItem {
  ...
  /**
   * Offline settlement slip money leg (frm_settlement): cash | bank | anamat.
   * Gold lines map to Offline gfine; sfine is display-only until jewellery-expert SoT.
   */
  moneyBucket?: "cash" | "bank" | "anamat";
}
...
export function GoldSettlementTab() {
  ...
  // Auto-select invoice when party changes — do not overwrite a valid manual pick.
  useEffect(() => {
    if (partyInvoices.length === 0) {
      setSelectedInvoiceId((prev) => (prev === "" ? prev : ""));
      return;
    }
    setSelectedInvoiceId((prev) => {
      if (prev && partyInvoices.some((inv) => inv.id === prev)) return prev;
      return partyInvoices[0].id;
    });
  }, [partyInvoices]);
  ...
  const [activeFineGrams, setActiveFineGrams] = useState("");
  ...
  /** Offline money leg when kind=cash: cash | bank | anamat (advance). */
  const [activeMoneyBucket, setActiveMoneyBucket] = useState<"cash" | "bank" | "anamat">("cash");
  ...
  const partiesToSelect = useMemo(() => {
    if (partyType === "customer") {
      return people.filter(isCustomerParty);
    }
    if (partyType === "worker") {
      return people.filter(isKarigarParty);
    }
    if (partyType === "vendor") {
      return people.filter(isSupplierParty);
    }
    return [];
  }, [people, partyType]);

  // Set default party when type changes — keep selection if still valid.
  useEffect(() => {
    if (partiesToSelect.length === 0) {
      setPartyId((prev) => (prev === "" ? prev : ""));
      return;
    }
    setPartyId((prev) => {
      if (prev && partiesToSelect.some((p) => p.id === prev)) return prev;
      return partiesToSelect[0].id;
    });
  ...
  const calculatedActiveFine = useMemo(() => {
    const gross = Number(activeGrossGrams) || 0;
    if (gross <= 0) return null;
    try {
      const base = currentGoldCalculationRules();
      const settlementOn = isJewelleryCalcFeatureEnabled("settlementCalculation", base);
      const rules = settlementOn
        ? base
        : {
            ...base,
            featureFlags: {
      ...
      return computeFineGold(
        {
          module: "gold_settlement",
          grossMg: gramsToMg(gross),
          addMg: gramsToMg(Number(activeAddGrams) || 0),
          lessMg: gramsToMg(Number(activeLessGrams) || 0),
          tanchPct: Number(activePurity) || 0,
          wastagePct: Number(activeWastagePct) || 0,
          userFineMg: gramsToMg(Number(activeFineGrams) || 0),
        },
        rules,
      );
    } catch {
  ...
  const fineIsUserControlled = calculatedActiveFine?.snapshot.userControlledFine === true;

  const calculatedActiveFineGrams = fineIsUserControlled
    ? Number(activeFineGrams) || 0
    : calculatedActiveFine
      ? Number(mgToGrams(calculatedActiveFine.fineMg))
      : 0;

  // When ERP calculation is active, keep the fine field in sync with the named method (visible, not silent).
  useEffect(() => {
    if (!fineIsUserControlled && calculatedActiveFine) {
      setActiveFineGrams(mgToGrams(calculatedActiveFine.fineMg));
    }
  }, [fineIsUserControlled, calculatedActiveFine?.fineMg]);
  ...
  const handleAddItemRow = () => {
    ...
    if (activeKind === "gold") {
      ...
      const newItem: SettlementItem = {
        ...
        formulaSnapshot: calculatedActiveFine?.snapshot,
  ...
  /** Offline frm_settlement five-bucket slip (display from existing lines — no new SoT). */
  const offlineFiveBucket = useMemo(() => {
    const signed = (it: SettlementItem, rupees: number) =>
      (it.direction === "Jama" ? 1 : -1) * rupees;
    let gfineGrams = 0;
    let cashRupees = 0;
    let bankRupees = 0;
    let anamatRupees = 0;
    for (const it of items) {
      if (it.kind === "gold") {
        gfineGrams += signed(it, it.fineGrams || 0);
        continue;
      }
      const amt = it.amountRupees || 0;
      const bucket =
        it.moneyBucket ||
        (it.description.toLowerCase().includes("anamat") ||
        it.description.toLowerCase().includes("advance")
          ? "anamat"
          : it.description.toLowerCase().includes("bank") ||
              it.description.toLowerCase().includes("upi") ||
              it.description.toLowerCase().includes("transfer")
            ? "bank"
            : "cash");
      if (bucket === "anamat") anamatRupees += signed(it, amt);
      else if (bucket === "bank") bankRupees += signed(it, amt);
      else cashRupees += signed(it, amt);
    }
    return {
      gfineGrams,
      sfineGrams: 0,
  ...
  const handleVoucherSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Cash settlement policy + live cash book — validate before any gold posts.
      const cashOutflowPaise = slipNetCashPaise < 0 ? Math.abs(slipNetCashPaise) : 0;
      const cashMethod =
        paymentMode === "upi" ||
        paymentMode === "bank" ||
        paymentMode === "card" ||
        paymentMode === "cheque"
          ? paymentMode
          : "cash";
      if (cashOutflowPaise > 0 || Math.abs(slipNetCashPaise) > 0) {
        const { useBusinessRules } = await import("@/lib/business-rules-store");
        if (!useBusinessRules.getState().isEnabled("allow_cash_settlement")) {
          throw new Error(
            "Cash settlement is disabled in Business Rules. Remove cash lines or enable Allow Cash Settlement.",
          );
        }
      }
      if (cashOutflowPaise > 0 && cashMethod === "cash" && cashBookEnabled()) {
        const { assertTransactionCashAvailable } = await import(
          "@/lib/transaction-ledger-guards"
        );
        assertTransactionCashAvailable(cashOutflowPaise);
      }
      ...
      const { captureSettlementCreate } = await import("@/lib/offline");
      const queued = await captureSettlementCreate(settlementInput);
      if (queued.mode === "queued") {
        toast.message(
          "Settlement saved offline — Pending Sync. Gold and cash post only after reconnect (no local duplicate posts).",
        );
        setOpen(false);
        setItems([]);
        setNotes("");
      ...
      // Cash before gold: never leave gold posted if cash outflow fails.
      if (
        Math.abs(slipNetCashPaise) > 0 &&
        finalPartyId &&
        partyType !== "internal" &&
        linkUse !== "invoice"
      ) {
        ...
        if (cashBookEnabled()) {
          await postMoneyVoucher({
            kind: slipNetCashPaise > 0 ? "receipt" : "payment",
            partyId: finalPartyId,
            amountPaise: Math.abs(slipNetCashPaise),
            method: cashMethod as import("@/lib/billing-store").PaymentMode,
            narration: `Settlement ${voucherNo} cash line`,
            reference: voucherNo,
            source: "gold_settlement_cash",
            sourceId: `${voucherNo}_cash`,
          });
        }
      }
      ...
      if (
        ...
        for (const it of items) {
          ...
          if (it.kind === "cash") {
            let mode: any = "cash";
            if (it.moneyBucket === "bank") {
              mode = "bank";
            } else if (it.moneyBucket === "anamat") {
              mode = "anamat";
            } else {
              ...
              if (descLower.includes("upi")) {
                ...
                mode = "card";
              } else if (descLower.includes("anamat") || descLower.includes("advance")) {
                mode = "anamat";
              }
            }
      ...
      for (const it of goldRows) {
        ...
        if (partyType === "worker") {
          ...
          // Customer custody: vault and customer move together (canonical liability model).
          netFineMg = factorSign * (itemFineMg + itemFineMg);
          deltas = {
            vault: factorSign * itemFineMg,
            customer: factorSign * itemFineMg,
          };
        } else {
          // Vendor / other party metal
            ...
            <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto rounded-md border border-border bg-card p-6 shadow-2xl">
              <DialogHeader>
                <DialogTitle className="font-serif text-xl text-yellow-600 flex items-center gap-2">
                  <Coins className="h-6 w-6 text-gold animate-bounce" /> Gold Payment &
                  Settlement Voucher
                </DialogTitle>
              </DialogHeader>
              ...
              <form onSubmit={handleVoucherSubmit} className="space-y-6 pt-2">
                ...
                <div className="bg-muted/30 p-4 rounded-md border border-border space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    {activeKind === "cash" && (
                      <>
                        <div>
                          <Label className="text-xs font-semibold">Offline money leg *</Label>
                          <Select
                            value={activeMoneyBucket}
                            onValueChange={(v) =>
                              setActiveMoneyBucket(v as "cash" | "bank" | "anamat")
                            }
                          >
                            <SelectTrigger className="mt-1 bg-background">
                              <SelectValue />
                            ...
                            <SelectContent>
                              ...
                              <SelectItem value="anamat">Anamat (cash advance)</SelectItem>
                            </SelectContent>
                          </Select>
                          <p className="text-[10px] text-muted-foreground mt-1">
                            Maps Offline settlement buckets. Anamat posts as cash advance — not a
                            separate metal ledger.
                          </p>
                        </div>
                        <div>
                      ...
                      <div className="grid grid-cols-2 gap-1 font-mono text-[11px] text-muted-foreground p-1 bg-background/50 border border-border/80 rounded col-span-2">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5">
                            <span>Fine:</span>
                            {fineIsUserControlled ? (
                              <Badge variant="outline" className="text-[9px] px-1 py-0 h-4">
                                User-entered
                              </Badge>
                            ) : (
                              <Badge className="text-[9px] px-1 py-0 h-4 bg-amber-600/90">
                                Calculated
                              </Badge>
                            )}
                          </div>
                          {fineIsUserControlled ? (
                            <Input
                              type="number"
                              step="0.001"
                              value={activeFineGrams}
                              onChange={(e) => setActiveFineGrams(e.target.value)}
                              placeholder="0.000"
                              className="h-7 bg-background font-mono text-amber-600 font-bold"
                              data-testid="settlement-user-fine"
                            />
                          ) : (
                          ...
                          {calculatedActiveFine?.snapshot.formulaLabel && (
                          ...
                          {items.map((it) => (
                            <tr key={it.id} className="border-b border-border/60 hover:bg-muted/10">
                              <td className="p-2">
                                ...
                                {it.kind === "cash" && it.moneyBucket ? (
                                  <div className="text-[10px] text-muted-foreground uppercase">
                                    Offline leg: {it.moneyBucket}
                                  </div>
                                ) : null}
                                {it.huid && (
                ...
                <div className="rounded-md border border-border bg-muted/20 p-4 space-y-2">
                  <div className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
                    Offline settlement slip (five buckets)
                  </div>
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-sm">
                    <div>
                      <p className="text-[10px] uppercase text-muted-foreground">GFine</p>
                      <p className="font-mono font-semibold">
                        {offlineFiveBucket.gfineGrams.toFixed(3)} g
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase text-muted-foreground">SFine</p>
                      <p className="font-mono font-semibold text-muted-foreground">
                        —{" "}
                        <span className="text-[10px] font-sans normal-case">
                          (not separate SoT)
                        </span>
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase text-muted-foreground">Cash</p>
                      <p className="font-mono font-semibold">
                        ₹{offlineFiveBucket.cashRupees.toFixed(2)}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase text-muted-foreground">Anamat</p>
                      <p className="font-mono font-semibold">
                        ₹{offlineFiveBucket.anamatRupees.toFixed(2)}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase text-muted-foreground">Bank</p>
                      <p className="font-mono font-semibold">
                        ₹{offlineFiveBucket.bankRupees.toFixed(2)}
                      </p>
                    </div>
                  </div>
                  <p className="text-[10px] text-muted-foreground">
                    Display mirrors Offline frm_settlement columns from this voucher&apos;s lines.
                    Posts still use gold_ledger + money vouchers (no duplicate settle ledger).
                  </p>
                </div>
