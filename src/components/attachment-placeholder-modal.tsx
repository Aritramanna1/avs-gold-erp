...
export type AttachmentPlaceholderModalProps = {
  ...
  /** When true, a file must be uploaded to R2 — metadata-only saves are blocked. */
  requireR2Upload?: boolean;
  /** Portal session hint for multi-firm storage path resolution. */
...
export function AttachmentPlaceholderModal({
  ...
  const helpText = requireR2Upload
    ? "Upload a file to store it in Cloudflare R2. Notes alone cannot be saved without an uploaded document."
  ...
  const handleSave = async () => {
    ...
    try {
      const hasStoredFile = !!(existing?.storagePath && existing?.bucket);

      if (requireR2Upload && !selectedFile && !hasStoredFile) {
        toast.error("Choose a file to upload. KYC documents must be stored in R2.");
        return;
      }
      ...
      } else if (!requireR2Upload) {
        // Metadata-only edit (note / filed toggle) on an existing record.
        save(entityType, entityId, docKey, { filed, note: note.trim(), fileName });
      } else {
        // Notes-only edit on an existing R2-backed record.
        save(entityType, entityId, docKey, { note: note.trim(), fileName });
      }
...
export function AttachmentButton({
  ...
  const filed = requireR2Upload
    ? !!(rec?.storagePath && rec?.bucket)
    : !!rec?.filed;
  return (
    <>
      <Button
      ...
      <AttachmentPlaceholderModal
        ...
        docLabel={docLabel}
        onSaved={onSaved}
        requireR2Upload={requireR2Upload}
        portalType={portalType}
      />
    </>
  );
