...
import { erpAppUrl, isErpEntryPath, isPlatformCmsPath, marketingAppUrl } from "@/lib/public-origin";

function CtaLink({ href, label }: { href: string; label: string }) {
  if (isPlatformCmsPath(href)) {
    return <a href={marketingAppUrl(href)}>{label}</a>;
  }
  if (isErpEntryPath(href)) {
    return <a href={erpAppUrl(href)}>{label}</a>;
  }
                  ...
                  <div>
                    ...
                    <div className="mt-8 flex flex-wrap gap-3">
                      {section.primary_cta && (
                        ...
                        >
                          <CtaLink href={section.primary_cta.href} label={section.primary_cta.label} />
                        </Button>
                      )}
                      {section.secondary_cta && (
                        <Button asChild size="lg" variant="outline">
                          <CtaLink href={section.secondary_cta.href} label={section.secondary_cta.label} />
                        </Button>
                      )}
                    </div>
                  </div>
