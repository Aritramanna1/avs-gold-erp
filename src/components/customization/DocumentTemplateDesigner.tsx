...
export function DocumentTemplateDesigner() {
  ...
  const handleApplyFamilyGroupToAllDocuments = async (family: TemplateFamily) => {
    try {
      const familyName = TEMPLATE_FAMILY_LABELS[family]?.name || family;
      for (const dt of allDocTypes) {
        const familyDefaults = DEFAULT_TEMPLATES[dt] ?? DEFAULT_TEMPLATES.gst_invoice;
        const match = familyDefaults.find((t) => t.family === family) ?? familyDefaults[0];
        if (match) {
          const existing = getForDocType(dt, "a4");
          if (existing && !existing.isBuiltin) {
            await update(existing.id, {
              family,
              name: `${PRINT_DOC_LABELS[dt]} (${familyName})`,
              sections: match.sections,
            });
          }
        }
      ...
      setActiveTemplate({
        ...activeTemplate,
        family,
        name: `${PRINT_DOC_LABELS[selectedDocType]} (${familyName})`,
      });
      toast.success(`Applied "${familyName}" group across ALL 35 ERP document templates!`);
    } catch (err: any) {
      toast.error(err?.message || "Failed to apply template family globally.");
    }
  };
            ...
            <CardHeader className="pb-3 pt-4 px-4">
              <div className="flex items-center justify-between">
                <CardTitle className="text-xs font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
                  <Sparkles className="h-3.5 w-3.5 text-amber-500" /> Unified Template Group (Family)
                </CardTitle>
              ...
              <CardDescription className="text-xs">
                Select a cohesive theme family. You can apply it to this single document or uniformly across all 35 ERP documents.
              </CardDescription>
            </CardHeader>
            <CardContent className="px-4 pb-4 space-y-3">
              <div className="space-y-1">
                <Label className="text-xs text-muted-foreground">Active Template Family</Label>
                <Select
                  value={activeTemplate.family || "modern_professional"}
              ...
              <p className="text-[11px] text-muted-foreground italic">
                {TEMPLATE_FAMILY_LABELS[activeTemplate.family || "modern_professional"]?.description}
              </p>
              <div className="pt-1 flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => handleApplyFamilyGroupToAllDocuments(activeTemplate.family || "modern_professional")}
                  className="w-full text-xs h-8 bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 border-amber-500/30 font-medium"
                >
                  <Sparkles className="h-3 w-3 mr-1 text-amber-500" />
                  Apply "{TEMPLATE_FAMILY_LABELS[activeTemplate.family || "modern_professional"]?.name}" Group to ALL Documents
                </Button>
              </div>
            </CardContent>
          </Card>
