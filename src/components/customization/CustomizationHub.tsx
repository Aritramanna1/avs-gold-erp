...
import { useSettings, DROPDOWN_LABELS, type DropdownKey, flushSettingsPersistence } from "@/lib/settings-store";
...
export const CUSTOMIZATION_CATEGORIES = [
  {
    ...
    icon: Layers,
    description: "Default catalog template, page sizes, field visibility, branding, and template designer",
  },
  {
    key: "trade_config",
    label: "Trade & System Config",
    icon: Sliders,
    description: "Production configuration master: Features, General, Master, Tagging & Vouchers",
  },
  {
    key: "language",
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <Tabs value={currentTab} onValueChange={handleTabChange}>
        ...
        {/* 0. Trade & System Configuration Master */}
        <TabsContent value="trade_config" className="mt-4 space-y-4">
          <LegacyParityConfigurationPanel />
        </TabsContent>
        ...
        <TabsContent value="calculations" className="mt-4 space-y-6">
          <GoldCalculationRulesPanel />
          <CalculationsContent />
        </TabsContent>
        ...
        <TabsContent value="gold" className="mt-4 space-y-6">
          <GoldCalculationRulesPanel />
  ...
  const handleImportBundle = async (file: File) => {
    try {
      ...
      const bundle = JSON.parse(text) as {
        dropdowns?: typeof settings.dropdowns;
        formsMetadata?: typeof settings.formsMetadata;
        terminologyOverrides?: Record<string, string>;
        activeTerminologyPack?: string;
      };
      if (bundle.dropdowns) {
        useSettings.setState({ dropdowns: bundle.dropdowns });
      }
      if (bundle.formsMetadata) {
        useSettings.setState({ formsMetadata: bundle.formsMetadata });
      }
      if (bundle.terminologyOverrides || bundle.activeTerminologyPack) {
        useTerminology.setState({
          customOverrides: bundle.terminologyOverrides ?? terminology.customOverrides,
          activePack: (bundle.activeTerminologyPack ??
            terminology.activePack) as typeof terminology.activePack,
        });
      }
      await flushSettingsPersistence();
      toast.success("Configuration bundle imported.");
    } catch {
      toast.error("Invalid configuration bundle JSON.");
    }
  };
