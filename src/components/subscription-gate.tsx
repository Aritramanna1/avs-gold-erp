...
export function SubscriptionGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        if (resolved) {
          const orgId = useTenantContext.getState().activeOrganizationId;
          await resolveSubscriptionAccess({ organizationId: orgId });
        }
      } catch (e) {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    // Bootstrap already resolved subscription once; interval only for long-lived rechecks.
  ...
  const isPlatformOwner = useAuthorizationContext.getState().context?.is_platform_owner === true;
  if (isPlatformOwner) {
    return <>{children}</>;
  }
