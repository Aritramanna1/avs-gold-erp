 ...
 * internet). The web app now relies on Supabase for authenticated access and
 * cloud-backed data, so this gate only reports service availability.
...
export function BackendGate({ children }: { children: ReactNode }) {
  useEffect(() => {
    supabase.auth.getSession().then(({ error }) => {
      if (error) {
        console.warn("[BackendGate] Cloud authentication check failed:", error.message);
      }
    });
  }, []);
