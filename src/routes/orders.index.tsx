...
export const Route = createFileRoute("/orders/")({
  head: () => ({ meta: [{ title: "Orders · AVS ERP" }] }),
...
function OrdersListPage() {
  ...
  useEffect(() => {
    ...
    fetchOrdersPage({
      ...
      .then((result) => {
        if (cancelled) return;
        // Owners already queried firm-wide (ordersBranchId null); branchFilter
        // only narrows for branch-locked staff.
        setPageOrders(ordersBranchId ? branchFilter(result.orders) : result.orders);
    ...
    // branchFilter is memoized on activeBranchId; ordersBranchId covers the same.
  }, [ordersBranchId, page, query, statusF, typeF, branchFilter]);

  // Global search deep link: open the order detail route.
  useEffect(() => {
    const id = search.selected;
    if (!id) return;
    void navigate({ to: "/orders/$id", params: { id }, replace: true });
  }, [search.selected]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="rounded-md border border-border bg-card p-4 mb-4 flex flex-col sm:flex-row gap-3">
        ...
        <Select
          value={statusF}
          onValueChange={(v) => updateSearch({ status: v as typeof statusF, page: 1 })}
        >
        ...
        <Select
          value={typeF}
          onValueChange={(v) => updateSearch({ type: v as typeof typeF, page: 1 })}
        >
