...
import { exportBlobFile } from "@/lib/report-engine";
...
function downloadGstCsv(content: string, filename: string) {
  exportBlobFile(filename, content, "text/csv;charset=utf-8;", ".csv");
}
...
function GstReturnsPage() {
  ...
  const [from, setFrom] = useState(toDateInputValue(yearAgo.getTime()));
  const [to, setTo] = useState(toDateInputValue(today.getTime()));
  const [invoicesReady, setInvoicesReady] = useState(() => invoices.length > 0);
  ...
  useEffect(() => {
    ...
    void refreshInvoices()
      .catch(() => undefined)
      .finally(() => {
        if (!cancelled) setInvoicesReady(true);
      });
    return () => {
      cancelled = true;
    };
  }, [refreshInvoices, invoices.length, initialLoadDone]);
  ...
  function exportGSTR1() {
    ...
    downloadGstCsv(generateGSTR1CSV(periodInvoices), `GSTR1_B2B_${from}_to_${to}.csv`);
  ...
  function exportGSTR3B() {
    ...
    downloadGstCsv(generateGSTR3BCSV(summary), `GSTR3B_Summary_${from}_to_${to}.csv`);
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <Button
              ...
              data-testid="report-export-gstr3b"
              disabled={!invoicesReady || summary.invoiceCount === 0}
            >
