...
import { captureOrderCreate } from "@/lib/offline";
...
import { gramsToMg, parsePurity } from "@/lib/gold";
...
export const Route = createFileRoute("/orders/new")({
  head: () => ({ meta: [{ title: "Create Order · AVS ERP" }] }),
...
interface LineForm {
  ...
  perPieceG: string;
  /** Offline Less wt (g) — optional. */
  lessG: string;
  /** Offline Add wt (g) — Net = Gr − Less + Add. */
  addG: string;
  /** Expected wastage % (Offline Wstg). */
  wastagePct: string;
  /** Labour / majuri ₹ for this line (Offline making). */
  labourRupees: string;
...
interface FormState {
  ...
  /** Cash advance received at booking (Offline-style booking money). */
  cashAdvanceRupees: string;
...
function normalizeLine(raw: Partial<LineForm> & { grossG?: string }): LineForm {
  ...
  return {
    ...
    lessG: raw.lessG ?? "",
    addG: raw.addG ?? "",
    wastagePct: raw.wastagePct ?? "",
    labourRupees: raw.labourRupees ?? "",
...
function NewOrderPage() {
    ...
    () =>
      lines.map((l) => {
        ...
        return {
          ...
          purity,
          quantity,
          perPieceFineMg: fineGoldMgConfigured(
            perPieceGrossMg,
            purity,
            currentGoldCalculationRules(),
          ),
          fineMg: fineGoldMgConfigured(grossMg, purity, currentGoldCalculationRules()),
        };
      }),
    [lines],
  );
  ...
  const calc = useMemo(() => {
    ...
    const goldReceivedGrossFineMg = fineGoldMgConfigured(
      goldReceivedMg,
      goldReceivedPurity,
      currentGoldCalculationRules(),
    );
  ...
  async function submit() {
    ...
    try {
      if (!isOnline()) {
        if (!form.customerId || !form.productionType) return;
        const customerId = form.customerId;
        const { type, productionType } = resolveProductionType(form.productionType);
        const customerKnown = people.some((p) => p.id === customerId);
        const queued = await captureOrderCreate({
          customerId,
          customerLocalId: customerKnown ? undefined : customerId,
          goldReceived:
            calc.goldReceivedMg > 0
              ? {
                  netFineMg: calc.goldReceivedFineMg,
                  goldGrossMg: calc.goldReceivedMg,
                  goldPurity: calc.goldReceivedPurity,
          ...
          order: {
            ...
            items: lines.map((l, i) => ({
              ...
              lessMg: safeMg(l.lessG),
              addMg: safeMg(l.addG) || undefined,
              netMg: Math.max(0, lineCalcs[i].grossMg - safeMg(l.lessG) + safeMg(l.addG)),
              fineMg: lineCalcs[i].fineMg,
              expectedWastagePct: Number(l.wastagePct) || 0,
              expectedWastageMg: Math.round(
                (lineCalcs[i].grossMg * (Number(l.wastagePct) || 0)) / 100,
              ),
              labourRupees: Number(l.labourRupees) || undefined,
              remarks: l.remarks || undefined,
              lineId: l.lineId,
            })),
            advance: {
              cashPaise: Math.round((Number(form.cashAdvanceRupees) || 0) * 100),
            ...
            timeline: [{ ts: Date.now(), label: "Order captured offline — Pending Sync" }],
          },
        });
        if (queued.mode === "queued") {
          toast.message(
            "Order saved offline — Pending Sync. Gold is not posted to the vault until sync succeeds.",
          );
  ...
  async function submitInternal() {
    ...
    if (calc.goldReceivedMg > 0) {
      const entry = await appendLedger({
        ...
        netFineMg: calc.goldReceivedFineMg + calc.goldReceivedFineMg,
        deltas: { vault: calc.goldReceivedFineMg, customer: calc.goldReceivedFineMg },
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto pb-28 md:pb-8">
      ...
      >
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Less Wt (g)">
                  <Input
                    value={line.lessG}
                    onChange={(e) => setLine(i, { lessG: e.target.value })}
                    placeholder="0.000"
                    inputMode="decimal"
                    className="h-11 sm:h-9"
                  />
                </Field>
                <Field label="Add Wt (g)">
                  <Input
                    value={line.addG}
                    onChange={(e) => setLine(i, { addG: e.target.value })}
                    placeholder="0.000"
                    inputMode="decimal"
                    className="h-11 sm:h-9"
                  />
                ...
                <Field label="Wastage %">
                  <Input
                    ...
                    onChange={(e) => setLine(i, { wastagePct: e.target.value })}
                    placeholder="0"
                    inputMode="decimal"
                    className="h-11 sm:h-9"
                  />
                </Field>
                <Field label="Labour / Majuri (₹)">
                  <Input
                    value={line.labourRupees}
                    onChange={(e) => setLine(i, { labourRupees: e.target.value })}
                    placeholder="0"
                    inputMode="decimal"
                    className="h-11 sm:h-9"
                  />
        ...
        <Section title="Gold Received">
          <div className="grid sm:grid-cols-2 gap-4">
            <Field label="Cash Advance (₹)">
              <Input
                value={form.cashAdvanceRupees}
                onChange={(e) => setForm((f) => ({ ...f, cashAdvanceRupees: e.target.value }))}
                placeholder="0 — booking money received"
                inputMode="decimal"
              />
            </Field>
        ...
        <div className="ornexa-sticky-action-bar md:static md:border-0 md:bg-transparent md:p-0 md:backdrop-filter-none flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-end gap-2">
          <Link to="/orders" className="sm:w-auto">
            <Button type="button" variant="ghost" className="w-full min-h-[var(--touch-target)] sm:min-h-0">
              Cancel (Esc)
            </Button>
          </Link>
          <Button
            ...
            className="gap-2 w-full sm:w-auto min-h-[var(--touch-target)]"
      ...
      <PersonFormDialog
        open={!!quickAdd}
        initial={null}
        defaultType={quickAdd?.type ?? "customer"}
  ...
  return (
    <div className="rounded-md border border-gold/40 bg-gold/5 p-4 flex items-start justify-between gap-3">
      <div className="flex items-start gap-3 min-w-0">
        <div className="h-10 w-10 rounded-full bg-primary text-primary-foreground grid place-items-center font-serif font-bold shrink-0">
