...
export const Route = createFileRoute("/billing/credit-notes/$id")({
  head: () => ({ meta: [{ title: "Credit Note · AVS Gold ERP" }] }),
...
function CreditNoteDetail() {
  ...
  async function handleCancel() {
    ...
    try {
      ...
      toast.error(e?.message || "Failed to cancel credit note.");
    } finally {
      setCancelling(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-100 p-4 md:p-8 print:bg-white print:p-0">
      ...
      >
        <div className="flex items-start justify-between border-b-4 border-amber-500 pb-4 mb-4">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold text-amber-600">CREDIT NOTE</div>
            <div className="text-xs text-neutral-500 font-mono">{note.creditNoteNo}</div>
      ...
      <AlertDialog open={cancelOpen} onOpenChange={setCancelOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cancel Credit Note {note.creditNoteNo}?</AlertDialogTitle>
            <AlertDialogDescription>
