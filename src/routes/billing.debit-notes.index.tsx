...
import { type DebitNote, useDebitNotes } from "@/lib/billing-documents-store";
...
export const Route = createFileRoute("/billing/debit-notes/")({
  head: () => ({ meta: [{ title: "Debit Notes · AVS Gold ERP" }] }),
  component: DebitNotesIndex,
});

function DebitNotesIndex() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setLoadError(null);
    Promise.all([fetchDebitNotes(q), fetchRecentInvoiceOptions(), fetchActiveCustomerOptions()])
      ...
      .catch((error: any) => {
        if (!live) return;
        setLoadError(error?.message ?? "Could not load debit notes.");
      })
      .finally(() => {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      toast.error(e?.message || "Failed to issue debit note.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <PageHeader
        ...
        actions={
          can("billing.create") ? (
            <Button className="gap-2" onClick={() => setOpen(true)} data-testid="debit-note-new">
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        <div className="relative max-w-sm mb-3">
          ...
          <Input
            ...
            placeholder="Search debit note, customer, invoice"
            className="pl-9"
          />
        </div>
        {loadError ? (
          <div className="py-6 text-center text-sm">
            ...
            <Button
              ...
              onClick={() => {
                ...
                void fetchDebitNotes(q).then(setNotes);
              }}
            >
              Retry
            </Button>
          ...
          <p className="text-sm text-muted-foreground py-6 text-center">Loading debit notes...</p>
        ) : list.length === 0 ? (
          <p className="text-sm text-muted-foreground py-6 text-center">No debit notes found.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="text-xs uppercase text-muted-foreground">
                <tr className="border-b border-border">
                  <th className="text-left py-2">Debit Note</th>
                ...
                {list.map((n) => (
                  <tr key={n.id} className="border-b border-border/60 hover:bg-background/30">
                    ...
                    <td className="text-right">
                      <Link to="/billing/debit-notes/$id" params={{ id: n.id }}>
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Reason for debit note (required)"
                rows={3}
              />
            </div>
          </div>
