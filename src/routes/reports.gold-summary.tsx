 ...
 */
/** Canonical manufacturing-gold rollup across a set of bills — exported so
 * the CEO Dashboard's Manufacturing Summary card reuses this exact
 * computation instead of re-deriving it, per branch or company-wide. */
export function computeManufacturingGoldTotals(
...
function GoldSummaryPage() {
  ...
  const totals = useMemo(() => computeManufacturingGoldTotals(branchBills), [branchBills]);
  ...
  function handleCSV() {
    ...
    const billsRows = branchBills
      .filter((b) => b.closingBalanceMg !== 0)
      .map((b) => [
        b.billNo,
        b.karigarName ?? "—",
        mgToGrams(b.totalGoldIssuedFineMg),
        mgToGrams(b.totalGoldReturnedFineMg),
        mgToGrams(b.actualWastageFineMg),
        mgToGrams(b.closingBalanceMg),
      ]);
