...
function bucketFor(days: number): AgeBucket {
  return (
    BUCKETS.find((b) => days >= b.minDays && (b.maxDays === null || days <= b.maxDays)) ??
...
function InventoryAgeingPage() {
  ...
  const available = useMemo(() => {
    ...
    return base.filter(
      (i) =>
        (i as StockItem & { branchId?: string }).branchId === selectedBranchId || !selectedBranchId,
    );
            ...
            <tbody>
              {buckets.map((b) => (
                <tr key={b.label} className="border-b border-border last:border-0">
                  <td className="p-3">
                    <Badge variant={b.minDays >= 181 ? "destructive" : "secondary"}>
                      {b.label}
                    </Badge>
                  </td>
