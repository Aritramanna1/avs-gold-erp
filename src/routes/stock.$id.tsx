...
function StockDetail() {
  ...
  const photoCount = countStockProductPhotos(item.id);
  const hasPhoto = stockItemHasProductPhoto(item);
  const missingDetails = stockItemMissingDetails(item);
    ...
    <div data-testid="stock-detail-root" className="p-4 md:p-8 max-w-5xl mx-auto">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {item.status === "available" && (
              <Button
                ...
                onClick={() => {
                  try {
                    assertReadyStockSellable(item);
                    void navigate({ to: "/billing/new", search: { stockId: item.id } });
                  } catch (err) {
                    toast.error(err instanceof Error ? err.message : "Cannot bill this item");
                  }
                }}
              >
      ...
      <div className="grid md:grid-cols-[1fr_320px] gap-6">
        <div className="rounded-md border border-border bg-card p-5 space-y-4">
          {(!hasPhoto || missingDetails.length > 0) && (
            <div className="rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-sm text-amber-950 dark:text-amber-100">
              {!hasPhoto
                ? "Add at least one product photo below before marking available or billing."
                : null}
              {missingDetails.length > 0
                ? ` Missing details: ${missingDetails.join(", ")}.`
                : null}
              {hasPhoto ? ` ${photoCount} photo(s) on file.` : null}
            </div>
          )}
          {hasPhoto && missingDetails.length === 0 ? (
            <p className="text-xs text-muted-foreground">{photoCount} product photo(s) on file.</p>
          ) : null}
                ...
                onClick={() => {
                  if (toStatus === "available") {
                    try {
                      ...
                      toast.error(err instanceof Error ? err.message : "Cannot set available");
                      return;
                    }
                  }
                  void changeStatus(item.id, toStatus);
                }}
              >
                Set status
              </Button>
      ...
      <div className="mt-6">
        <h3 className="font-serif text-gold mb-2">Product photos *</h3>
        <p className="text-xs text-muted-foreground mb-3">
          Ready stock must have one or more clear photos of the piece. Upload below.
        </p>
