...
function PolishingPage() {
      ...
      <div className="p-4 md:p-8 max-w-3xl mx-auto text-center">
        ...
        <div className="rounded-md border border-dashed border-border bg-card/40 p-12 mt-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            ...
            your transaction module.
          </p>
          <Button
            className="mt-6 gap-2"
            onClick={async () => {
              try {
                const role = useSettings.getState().currentUserRole;
                await useBusinessRules.getState().setRule(
                  "enable_polishing_module",
                  true,
                  { id: null, email: null, role },
                );
              } catch (e) {
                console.error(e);
              }
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <PageHeader
        title="Polishing"
        subtitle="Manufacturing step — send work already in process (weight before), receive it back (weight after). Loss = difference. Does not issue gold from the vault."
        actions={
          ...
          <>
            <div className="block md:hidden divide-y divide-border">
              {ledgerRows.map((r) => (
                <button
                  key={r.polisherId}
                  type="button"
                  className={`w-full text-left p-3.5 ${selectedPolisherId === r.polisherId ? "bg-gold/5" : ""}`}
                  onClick={() => setSelectedPolisherId(r.polisherId)}
                  data-testid={`polishing-ledger-row-${r.polisherId}`}
                >
                  <p className="text-sm font-semibold">{r.polisherName}</p>
                  <p className="text-[11px] text-muted-foreground mt-1 font-mono">
                    Sent {mgToGrams(r.totalIssuedFineMg)} g · Pending{" "}
                    {mgToGrams(r.pendingFineMg)} g · {r.currentJobs} jobs
                  </p>
                </button>
              ))}
            </div>
            ...
            <table className="w-full min-w-[700px] text-sm table-fixed">
              <thead className="text-xs uppercase text-muted-foreground bg-background/40">
                <tr>
                  <th className="text-left px-4 py-2 w-[28%]">Polisher</th>
                  <th className="text-right px-4 py-2 w-[14%]">Total Sent</th>
                  <th className="text-right px-4 py-2 w-[14%]">Total Received</th>
                  <th className="text-right px-4 py-2 w-[14%]">Pending</th>
                  <th className="text-right px-4 py-2 w-[12%]">Current Jobs</th>
                  <th className="text-right px-4 py-2 w-[18%]">Last Transaction</th>
                </tr>
              </thead>
              <tbody>
                {ledgerRows.map((r) => (
