...
function DocumentVaultPage() {
  ...
  async function handleDownload(entry: FileVaultEntry) {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Failed to decrypt/load file — it may be corrupted.",
      );
    } finally {
      setBusyId(null);
    }
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            ...
            <Input
              ...
              placeholder="e.g. order, stock_item"
              className="w-48"
            />
          </div>
          <div className="flex flex-col gap-1">
              ...
              {entries.map((entry) => (
                <tr key={entry.id} className="border-b border-border last:border-0">
                  ...
                  <td className="p-3 text-muted-foreground">
                    {entry.entity_type
                      ? `${entry.entity_type} · ${entry.entity_id?.slice(0, 10)}`
                      : "—"}
                  </td>
                  ...
                  <td className="p-3 text-right">
                    <div className="flex gap-1 justify-end">
                      <Button
                        size="sm"
                        variant="outline"
                        disabled={busyId === entry.attachment_id}
