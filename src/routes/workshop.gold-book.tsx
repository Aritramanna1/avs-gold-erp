...
function WorkerGoldBookPage() {
  ...
  // Active view states
  const [activeTab, setActiveTab] = useDraft<"ledger" | "balances" | "new_entry">(
                  ...
                  {filteredEntries.length === 0 ? (
                    <tr>
                      <td colSpan={10} className="p-12 text-center text-muted-foreground italic">
                        No transactions match the filter criteria. Issue or return gold/materials to
                        populate ledger.
                      </td>
                    </tr>
                  ) : (
                    filteredEntries.map((e) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="p-4 font-mono font-bold text-amber-500">{e.entryNo}</td>
                          <td className="p-4">
                            <div className="font-bold">{e.workerName}</div>
                          </td>
                          <td className="p-4 max-w-[180px] truncate" title={e.particulars}>
                            <span className="font-semibold text-foreground">{e.particulars}</span>
                            {e.reference && (
          ...
          <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden">
            ...
            <form onSubmit={handleSubmitEntry} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-t border-border/40 pt-3">
                ...
                <div>
                  ...
                  <div className="bg-muted/40 font-mono border border-border rounded-xl px-3 py-2 text-sm font-semibold text-muted-foreground select-none">
                    {Math.max(0, (Number(formGrossG) || 0) - (Number(formLessG) || 0)).toFixed(3)} g
                  </div>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-border/40">
                ...
                >
                  {entryType === "given"
                    ? "Record Material Issue & Print"
                    : "Record Material Return & Print"}
                </Button>
              </div>
            </form>
          </div>
