    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="no-print sticky top-0 z-10 border-b border-border bg-card/80 backdrop-blur px-4 py-3 flex items-center justify-between">
        ...
        <Button
          onClick={() => {
            void (async () => {
              const { previewPrintEngineDocument } = await import("@/lib/native/document-output");
              const ok = await previewPrintEngineDocument("gold_receive_slip", job.id);
              if (!ok) {
                const { toast } = await import("sonner");
                toast.error("Could not generate receive slip PDF.");
              }
            })();
          }}
          className="gap-2"
