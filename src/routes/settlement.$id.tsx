...
function SettlementView() {
  ...
  const [settlementMethod, setSettlementMethod] = useState<"gold" | "cash" | "mixed">("gold");
  ...
  async function handleAddItem() {
    ...
    setAddingItem(true);
    try {
      const fineMg = calculateSettlementLineFine({ grossMg, purityPermille: purity }).fineMg;
      await addItem(s!.id, {
  ...
  async function handleRecordPayment() {
    ...
    // Record Gold Component if applicable
    if (settlementMethod === "gold" || settlementMethod === "mixed") {
      const grossG = Number(goldGramsStr) || 0;
      if (grossG > 0) {
        const goldGrossMg = gramsToMg(goldGramsStr);
        const purity = Number(goldPurityStr) || 0;
        if (purity <= 0) {
          toast.error("Enter a valid gold purity.");
          return;
        }
        const goldFineMg = calculateSettlementLineFine({
          grossMg: goldGrossMg,
          purityPermille: purity,
        }).fineMg;
        const goldValuePaise = calculateGoldValuePaise(goldFineMg, goldRatePerGramPaise);
        ...
        recordedAny = true;
      } else if (settlementMethod === "gold") {
        toast.error("Enter gold weight in grams.");
        return;
      }
    }

    // Record Cash Component if applicable
    if (settlementMethod === "cash" || settlementMethod === "mixed") {
      const amt = rupeesToPaise(payAmt);
      if (amt > 0) {
        await recordPayment(s!.id, {
          mode: payMode === "gold_exchange" ? "cash" : payMode,
          amountPaise: amt,
          reference: payRef || undefined,
        });
        recordedAny = true;
      } else if (settlementMethod === "cash") {
        toast.error("Enter a cash payment amount.");
        return;
      }
    }

    if (!recordedAny) {
      toast.error("Enter at least one payment component (Gold or Cash).");
      return;
    }
    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
      ...
      {!isFinalised && (
        <div className="rounded-md border border-border bg-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                type="button"
                size="sm"
                variant={settlementMethod === "gold" ? "default" : "outline"}
                className={
                  settlementMethod === "gold"
                    ? "bg-amber-600 hover:bg-amber-700 text-white font-bold"
                    : ""
                }
                onClick={() => {
                  setSettlementMethod("gold");
                  setPayMode("gold_exchange");
                }}
                data-testid="settlement-method-gold"
              >
                [GOLD]
              </Button>
              <Button
                ...
                data-testid="settlement-method-cash"
              >
                [CASH]
              </Button>
              <Button
                type="button"
                size="sm"
                variant={settlementMethod === "mixed" ? "default" : "outline"}
                className={
                ...
                onClick={() => {
                  setSettlementMethod("mixed");
                }}
                data-testid="settlement-method-mixed"
              >
                [MIXED]
              </Button>
            </div>
          ...
          {/* GOLD SECTION */}
          {(settlementMethod === "gold" || settlementMethod === "mixed") && (
            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-md space-y-2">
              <div className="text-xs font-semibold uppercase tracking-wider text-amber-500 flex justify-between">
                <span>Gold Component (Primary)</span>
                {Number(goldGramsStr) > 0 && (
                  <span className="font-mono">
                    Fine Gold:{" "}
                    {(
                      (Number(goldGramsStr) * (Number(goldPurityStr) || 916)) /
                      1000
                    ).toFixed(3)}{" "}
                    g
                  </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <Input
                  placeholder="Gold gross weight (g)"
                  value={goldGramsStr}
                ...
                <Select value={goldPurityStr} onValueChange={setGoldPurityStr}>
                  <SelectTrigger data-testid="settlement-gold-purity-select">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {COMMON_PURITIES.map((p) => (
                ...
                <Input
                  placeholder="Gold Reference / Note"
                  value={payRef}
          ...
          {/* CASH SECTION */}
          {(settlementMethod === "cash" || settlementMethod === "mixed") && (
            <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-md space-y-2">
              <div className="text-xs font-semibold uppercase tracking-wider text-emerald-500 flex justify-between">
                <span>Cash / Monetary Component (Secondary)</span>
                {Number(payAmt) > 0 && goldRatePerGramPaise > 0 && (
                  <span className="font-mono">
                    Rate: ₹{paiseToRupees(goldRatePerGramPaise)}/g · Gold Equiv:{" "}
                    {(
                      (rupeesToPaise(payAmt) / goldRatePerGramPaise) || 0
                    ).toFixed(3)}{" "}
                    g
                  </span>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                />
                <Input
                  placeholder="Cash Reference / UTR"
                  value={payRef}
                  onChange={(e) => setPayRef(e.target.value)}
                />
              </div>
          ...
          <div className="flex justify-end">
            ...
            >
              <Coins className="h-4 w-4" /> Record Settlement Payment
            </Button>
          </div>
        </div>
      )}
