...
function InvoicePrint() {
  ...
  const totalHallmarkChargesPaise =
    inv.items?.reduce((acc, it) => acc + (it.hallmarkChargesPaise || 0), 0) || 0;
          ...
          >
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="bg-purple-900 text-white uppercase font-serif text-[9px] tracking-wider">
                  <tr>
                    ...
                    {hasHallmarkCharges && (
                      <th className="p-3 text-right border-r border-purple-800">Hallmark</th>
                    )}
                    {hasOtherCharges && (
                ...
                <tfoot className="bg-purple-50/50 border-t-2 border-purple-900 font-mono text-[10px] font-bold text-purple-950">
                  <tr>
                    ...
                    {hasHallmarkCharges && (
                      <td className="p-3 text-right font-semibold">
                        ₹{paiseToRupees(totalHallmarkChargesPaise)}
                      </td>
                    )}
                    {hasOtherCharges && (
                      <td className="p-3 text-right font-semibold">
            ...
            <div className="grid grid-cols-2 gap-6 items-start mt-4">
              ...
              <div className="border border-purple-100 rounded-xl bg-purple-950 text-white p-5 space-y-2 font-mono text-xs shadow-md">
                ...
                {inv.gst === "gst3" ? (
                  // IGST-mode invoices store the full tax in sgstPaise with
                  // cgstPaise = 0 (see computeInvoiceTotals) — unconditionally
                  // labeling both lines "CGST"/"SGST (1.5%)" would print a
                  // ₹0.00 CGST line and mislabel the actual IGST amount as
                  // SGST, plus a hardcoded rate that doesn't reflect whatever
                  // GST rate is actually configured.
