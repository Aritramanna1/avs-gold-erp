...
export const Route = createFileRoute("/billing/estimates/")({
  head: () => ({ meta: [{ title: "Estimates · AVS ERP" }] }),
        ...
        {loadError ? (
          ...
          <>
            <div className="block md:hidden space-y-2 p-3">
              {rows.map((e) => (
                <div key={e.id} className="rounded-xl border border-border p-3.5 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="font-mono text-xs text-gold">{e.estimateNo}</p>
                      <p className="text-sm mt-0.5">{e.customerName}</p>
                      <p className="text-[11px] text-muted-foreground">
                        ₹ {paiseToRupees(e.grandTotalPaise)} · {e.status}
                      </p>
                    </div>
