...
function ExceptionReportPage() {
  ...
  function handleCSV() {
    const header = ["Severity", "Category", "Title", "Detail"];
    const data = filtered.map((e) => [e.severity, CATEGORY_LABELS[e.category], e.title, e.detail]);
    exportToCSV(`exception-report-${new Date().toISOString().slice(0, 10)}.csv`, [header, ...data]);
  }
