    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="no-print sticky top-0 z-10 border-b border-border bg-card/80 backdrop-blur px-4 py-3 flex items-center justify-between">
        ...
        <Button
          onClick={() => {
            void (async () => {
              const { previewStructuredReport } = await import("@/lib/native/document-output");
              await previewStructuredReport({
                title: `Conversion Slip ${record.batchNo}`,
                tables: [
                  [
                    ["Field", "Value"],
                    ["Batch", record.batchNo],
                    ["Source purity", String(record.sourcePurity)],
                    ["Destination purity", String(record.destPurity)],
                    ["Operator", record.operator],
                    ["Going out gross (g)", mgToGrams(record.sourceGrossMg && record.sourceGrossMg > 0 ? record.sourceGrossMg : record.inputFineMg)],
                    ["Coming in gross (g)", mgToGrams(record.destGrossMg && record.destGrossMg > 0 ? record.destGrossMg : record.actualOutputFineMg)],
                    ["Alloy added (g)", mgToGrams(record.alloyAddedMg)],
                    ["Conversion loss (g)", mgToGrams(record.conversionLossMg)],
                    ["Recovery (g)", mgToGrams(record.recoveryMg)],
                    ["Notes", record.notes || "—"],
                  ],
                ],
              });
              ...
              <tbody>
                <tr className="border-t border-gray-200">
                  <td className="p-2 font-medium">
                    Going out ({record.sourcePurity}‰)
                  </td>
                  <td className="p-2 text-right font-mono font-bold">
                    {mgToGrams(
                      record.sourceGrossMg && record.sourceGrossMg > 0
                        ? record.sourceGrossMg
                        : record.inputFineMg,
                    )}
                  </td>
                </tr>
                <tr className="border-t border-gray-200">
                  <td className="p-2 font-medium">
                    Coming in ({record.destPurity}‰)
                  </td>
                  <td className="p-2 text-right font-mono font-bold">
                    {mgToGrams(
                      record.destGrossMg && record.destGrossMg > 0
                        ? record.destGrossMg
                        : record.actualOutputFineMg,
                    )}
                  </td>
                </tr>
                <tr className="border-t border-gray-200">
                  <td className="p-2">Alloy added (total)</td>
                  <td className="p-2 text-right font-mono">
                    {mgToGrams(record.alloyAddedMg)} g
                  </td>
                </tr>
                {(record.alloyLines ?? []).map((line, i) => (
                  <tr key={`alloy-${i}`} className="border-t border-gray-200 bg-gray-50">
                    <td className="p-2 pl-6 text-xs">
                      ↳ {line.material}
                      {line.materialType ? ` (${line.materialType})` : ""}
                    </td>
                    <td className="p-2 text-right font-mono text-xs">
                      {mgToGrams(line.weightMg)} g
                    </td>
                  </tr>
                ))}
              </tbody>
