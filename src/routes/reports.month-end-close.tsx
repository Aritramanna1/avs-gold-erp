...
export const Route = createFileRoute("/reports/month-end-close")({
  head: () => ({ meta: [{ title: "Month-End Close · AVS Gold ERP" }] }),
      ...
      <div className="rounded-2xl border border-border bg-card p-4 mb-6 flex flex-wrap gap-4 items-end">
        ...
        <div className="flex flex-col gap-1 flex-1 min-w-48">
          ...
          <Input
            ...
            placeholder="e.g. Daily close reconciled, all vouchers posted"
          />
        </div>
        <Button
          onClick={handleLock}
          disabled={busy || !!lockFor(branchId, period)}
          className="gap-2"
        >
          {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Lock className="h-4 w-4" />} Lock
          Period
        </Button>
      </div>
