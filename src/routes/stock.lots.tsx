...
export const Route = createFileRoute("/stock/lots")({
  head: () => ({ meta: [{ title: "Lot & Batch Management · AVS Gold ERP" }] }),
  component: LotsPage,
});
...
function LotsPage() {
  ...
  const branchLots = useMemo(
    () =>
      lots
        .filter((l) => l.branchId === (selectedBranchId || "MAIN"))
...
function AssignItemsDialog({ lot, onClose }: { lot: StockLot | null; onClose: () => void }) {
  ...
  const assigned = useMemo(
    () => (lot ? items.filter((i) => i.lotId === lot.id) : []),
    [items, lot],
  );
